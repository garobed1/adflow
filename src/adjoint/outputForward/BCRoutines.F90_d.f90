...
! apply bc's for a single block
    use constants
    use blockpointersiff, only : nbocos, bctype, nviscbocos, w, dw, x,&
...
&   bmtk1, fw, aa, d2wall, bmti1, bmti2, s
    use utils_diff, only : setbcpointers, getcorrectfork
    use bcpointers_diff
    implicit none
...
!  planes, i.e. a 2d problem.
    use constants
    use blockpointersiff, only : bcdata, bcdatad
    use flowvarrefstateiff, only : viscous, eddymodel
    use bcpointers_diff, only : gamma1, gamma2, ww1, ww1d, ww2, ww2d, &
&   pp1, pp1d, pp2, pp2d, rlv1, rlv1d, rlv2, rlv2d, istart, jstart, &
&   isize, jsize, rev1, rev1d, rev2, rev2d
    implicit none
! subroutine arguments.
...
! to obtain the halo velocity.
      ...
      vnd = two*(temp*ww2d(i, j, ivx)+ww2(i, j, ivx)*bcdatad(nn)%norm(i&
&       , j, 1)+temp0*ww2d(i, j, ivy)+ww2(i, j, ivy)*bcdatad(nn)%norm(i&
&       , j, 2)+temp1*ww2d(i, j, ivz)+ww2(i, j, ivz)*bcdatad(nn)%norm(i&
&       , j, 3))
      vn = two*(ww2(i, j, ivx)*temp+ww2(i, j, ivy)*temp0+ww2(i, j, ivz)*&
&       temp1)
! determine the flow variables in the halo cell.
      ...
      ww1d(i, j, ivx) = ww2d(i, j, ivx) - temp1*vnd - vn*bcdatad(nn)%&
&       norm(i, j, 1)
      ww1(i, j, ivx) = ww2(i, j, ivx) - vn*temp1
      temp1 = bcdata(nn)%norm(i, j, 2)
      ww1d(i, j, ivy) = ww2d(i, j, ivy) - temp1*vnd - vn*bcdatad(nn)%&
&       norm(i, j, 2)
      ww1(i, j, ivy) = ww2(i, j, ivy) - vn*temp1
      temp1 = bcdata(nn)%norm(i, j, 3)
      ww1d(i, j, ivz) = ww2d(i, j, ivz) - temp1*vnd - vn*bcdatad(nn)%&
&       norm(i, j, 3)
      ww1(i, j, ivz) = ww2(i, j, ivz) - vn*temp1
...
!  planes, i.e. a 2d problem.
    use constants
    use blockpointersiff, only : bcdata
    use flowvarrefstateiff, only : viscous, eddymodel
    use bcpointers_diff, only : gamma1, gamma2, ww1, ww2, pp1, pp2, rlv1&
...
!  ad slightly easier.
    ...
    use blockpointersiff, only : bcdata, bcdatad
    use flowvarrefstateiff, only : viscous, eddymodel
    use bcpointers_diff, only : gamma0, gamma3, ww0, ww0d, ww3, ww3d, &
&   pp0, pp0d, pp3, pp3d, rlv0, rlv0d, rlv3, rlv3d, rev0, rev0d, rev3, &
&   rev3d, istart, jstart, isize, jsize
    implicit none
! subroutine arguments.
    integer(kind=inttype), intent(in) :: nn
! local variables.
...
! ww3 etc instead of ww2 and ww1.
    do ii=0,isize*jsize-1
      ...
      temp0 = bcdata(nn)%norm(i, j, 2)
      temp1 = bcdata(nn)%norm(i, j, 3)
      vnd = two*(temp*ww3d(i, j, ivx)+ww3(i, j, ivx)*bcdatad(nn)%norm(i&
&       , j, 1)+temp0*ww3d(i, j, ivy)+ww3(i, j, ivy)*bcdatad(nn)%norm(i&
&       , j, 2)+temp1*ww3d(i, j, ivz)+ww3(i, j, ivz)*bcdatad(nn)%norm(i&
&       , j, 3))
      vn = two*(ww3(i, j, ivx)*temp+ww3(i, j, ivy)*temp0+ww3(i, j, ivz)*&
&       temp1)
! determine the flow variables in the halo cell.
      ww0d(i, j, irho) = ww3d(i, j, irho)
      ww0(i, j, irho) = ww3(i, j, irho)
      temp1 = bcdata(nn)%norm(i, j, 1)
      ww0d(i, j, ivx) = ww3d(i, j, ivx) - temp1*vnd - vn*bcdatad(nn)%&
&       norm(i, j, 1)
      ww0(i, j, ivx) = ww3(i, j, ivx) - vn*temp1
      temp1 = bcdata(nn)%norm(i, j, 2)
      ww0d(i, j, ivy) = ww3d(i, j, ivy) - temp1*vnd - vn*bcdatad(nn)%&
&       norm(i, j, 2)
      ww0(i, j, ivy) = ww3(i, j, ivy) - vn*temp1
      temp1 = bcdata(nn)%norm(i, j, 3)
      ww0d(i, j, ivz) = ww3d(i, j, ivz) - temp1*vnd - vn*bcdatad(nn)%&
&       norm(i, j, 3)
      ww0(i, j, ivz) = ww3(i, j, ivz) - vn*temp1
...
!  ad slightly easier.
    ...
    use blockpointersiff, only : bcdata
    use flowvarrefstateiff, only : viscous, eddymodel
    use bcpointers_diff, only : gamma0, gamma3, ww0, ww3, pp0, pp3, rlv0&
...
! centerline. this routine does just the 1st level halo.
    use constants
    use bcpointers_diff, only : ww1, ww1d, ww2, ww2d, pp1, pp1d, pp2, &
&   pp2d, rlv1, rlv1d, rlv2, rlv2d, rev1, rev1d, rev2, rev2d, xx, xxd, &
...
! determine the unit vector in this direction.
      arg1d = 2*nnx*nnxd + 2*nny*nnyd + 2*nnz*nnzd
      ...
      tmp = one/result1
      nnxd = tmp*nnxd + nnx*tmpd
      nnx = nnx*tmp
      nnyd = tmp*nnyd + nny*tmpd
      nny = nny*tmp
      nnzd = tmp*nnzd + nnz*tmpd
      nnz = nnz*tmp
! determine twice the tangential velocity vector of the
! internal cell.
      tmpd = two*(nnx*ww2d(i, j, ivx)+ww2(i, j, ivx)*nnxd+nny*ww2d(i, j&
&       , ivy)+ww2(i, j, ivy)*nnyd+nnz*ww2d(i, j, ivz)+ww2(i, j, ivz)*&
&       nnzd)
      tmp = two*(ww2(i, j, ivx)*nnx+ww2(i, j, ivy)*nny+ww2(i, j, ivz)*&
&       nnz)
      vtxd = nnx*tmpd + tmp*nnxd
      vtx = tmp*nnx
      vtyd = nny*tmpd + tmp*nnyd
      vty = tmp*nny
      vtzd = nnz*tmpd + tmp*nnzd
      vtz = tmp*nnz
...
! centerline. this routine does just the 1st level halo.
    use constants
    use bcpointers_diff, only : ww1, ww2, pp1, pp2, rlv1, rlv2, rev1, &
&   rev2, xx, istart, jstart, isize, jsize
    use flowvarrefstateiff, only : viscous, eddymodel
    implicit none
! subroutine arguments.
    integer(kind=inttype), intent(in) :: nn
...
! centerline. this routine does just the 2nd level halo.
    use constants
    use bcpointers_diff, only : ww0, ww0d, ww3, ww3d, pp0, pp0d, pp3, &
&   pp3d, rlv0, rlv0d, rlv3, rlv3d, rev0, rev0d, rev3, rev3d, xx, xxd, &
&   istart, jstart, isize, jsize
    use flowvarrefstateiff, only : viscous, eddymodel
    implicit none
! subroutine arguments.
...
! determine twice the tangential velocity vector of the
! internal cell.
      tmpd = two*(nnx*ww3d(i, j, ivx)+ww3(i, j, ivx)*nnxd+nny*ww3d(i, j&
&       , ivy)+ww3(i, j, ivy)*nnyd+nnz*ww3d(i, j, ivz)+ww3(i, j, ivz)*&
&       nnzd)
...
! centerline. this routine does just the 2nd level halo.
    use constants
    use bcpointers_diff, only : ww0, ww3, pp0, pp3, rlv0, rlv3, rev0, &
&   rev3, xx, istart, jstart, isize, jsize
    use flowvarrefstateiff, only : viscous, eddymodel
    implicit none
! subroutine arguments.
    integer(kind=inttype), intent(in) :: nn
...
! condition the pointers already defined.
    ...
    use bcpointers_diff, only : ww0, ww0d, ww1, ww1d, ww2, ww2d, rlv0, &
...
! small near the wall.
      if (correctfork) then
        rhokd = ww2(i, j, itu1)*ww2d(i, j, irho) + ww2(i, j, irho)*ww2d(&
&         i, j, itu1)
...
! condition the pointers already defined.
    ...
    use blockpointersiff, only : bcdata
    use inputdiscretization, only : viscwallbctreatment
    use bcpointers_diff, only : ww0, ww1, ww2, rlv0, rlv1, rlv2, pp0, &
&   pp1, pp2, pp3, rev0, rev1, rev2, istart, jstart, isize, jsize
    use flowvarrefstateiff, only : viscous, eddymodel
    use iteration, only : currentlevel, groundlevel
    implicit none
    logical, intent(in) :: secondhalo, correctfork
...
&   pp3, pp3d, rev0, rev0d, rev1, rev1d, rev2, rev2d, istart, jstart, &
&   isize, jsize
    use flowvarrefstateiff, only : viscous, eddymodel, rgas, rgasd
    use iteration, only : currentlevel, groundlevel
    implicit none
! subroutine arguments.
    logical, intent(in) :: secondhalo, correctfork
...
! halo cell such that the average is the wall temperature.
      temp = ww2(i, j, irho)
      temp0 = pp2(i, j)/(rgas*temp)
      t2d = (pp2d(i, j)-temp0*(temp*rgasd+rgas*ww2d(i, j, irho)))/(rgas*&
&       temp)
      t2 = temp0
...
! (except for the k correction) is okay. take the slip
! velocity into account.
      temp0 = pp1(i, j)/(rgas*t1)
      ww1d(i, j, irho) = (pp1d(i, j)-temp0*(t1*rgasd+rgas*t1d))/(rgas*t1&
&       )
      ww1(i, j, irho) = temp0
      ww1d(i, j, ivx) = -ww2d(i, j, ivx)
      ww1(i, j, ivx) = -ww2(i, j, ivx) + two*bcdata(nn)%uslip(i, j, 1)
...
! already set
    ...
    use bcpointers_diff, only : ww0, ww1, ww2, rlv0, rlv1, rlv2, pp0, &
&   pp1, pp2, pp3, rev0, rev1, rev2, istart, jstart, isize, jsize
    use flowvarrefstateiff, only : viscous, eddymodel, rgas
    use iteration, only : currentlevel, groundlevel
    implicit none
! subroutine arguments.
    logical, intent(in) :: secondhalo, correctfork
...
!  bleed outflow.
    use constants
    use blockpointersiff, only : bcdata, bcdatad
    use bcpointers_diff, only : ww0, ww0d, ww1, ww1d, ww2, ww2d, pp0, &
&   pp0d, pp1, pp1d, pp2, pp2d, rlv0, rlv0d, rlv1, rlv1d, rlv2, rlv2d, &
&   rev0, rev0d, rev1, rev1d, rev2, rev2d, gamma2, isize, jsize, istart,&
&   jstart
    use flowvarrefstateiff, only : eddymodel, viscous
    implicit none
! subroutine arguments.
    logical, intent(in) :: secondhalo, correctfork
    integer(kind=inttype), intent(in) :: nn
...
! possible presence of a k-equation.
      ...
      pint = pp2(i, j)
      if (correctfork) then
        pintd = pintd - twothird*(ww2(i, j, itu1)*ww2d(i, j, irho)+ww2(i&
...
! and the speed of sound for the internal cell.
      temp = one/ww2(i, j, irho)
      rd = -(temp*ww2d(i, j, irho)/ww2(i, j, irho))
      r = temp
      a2d = gamma2(i, j)*(r*pintd+pint*rd)
      ...
      wed = ww2d(i, j, ivz)
      we = ww2(i, j, ivz)
      qned = nnx*ued + ue*nnxd + nny*ved + ve*nnyd + nnz*wed + we*nnzd
...
! velocity components, are extrapolated.
      if (r .le. 0.0_8 .and. (gamma2(i, j) .eq. 0.0_8 .or. gamma2(i, j) &
&         .ne. int(gamma2(i, j)))) then
        pwr1d = 0.0_8
      else
        pwr1d = gamma2(i, j)*r**(gamma2(i, j)-1)*rd
      end if
      pwr1 = r**gamma2(i, j)
      ssd = pwr1*pintd + pint*pwr1d
      ss = pint*pwr1
      acd = qned + two*ovgm1*ad
      ac = qne + two*a*ovgm1
! compute the state in the halo.
      pwx1d = (pexitd-pexit*ssd/ss)/ss
      pwx1 = pexit/ss
      if (pwx1 .le. 0.0_8 .and. (ovg .eq. 0.0_8 .or. ovg .ne. int(ovg))&
&     ) then
        ww1d(i, j, irho) = 0.0_8
      else
      ...
      pp1d(i, j) = pexitd
      pp1(i, j) = pexit
      temp = pexit/ww1(i, j, irho)
      arg1d = gamma2(i, j)*(pexitd-temp*ww1d(i, j, irho))/ww1(i, j, irho&
&       )
      arg1 = gamma2(i, j)*temp
      temp = sqrt(arg1)
      if (arg1 .eq. 0.0_8) then
        ad = 0.0_8
      else
      ...
      qnh = ac - two*a*ovgm1
      ww1d(i, j, ivx) = ued + nnx*(qnhd-qned) + (qnh-qne)*nnxd
      ww1(i, j, ivx) = ue + (qnh-qne)*nnx
      ww1d(i, j, ivy) = ved + nny*(qnhd-qned) + (qnh-qne)*nnyd
      ww1(i, j, ivy) = ve + (qnh-qne)*nny
      ww1d(i, j, ivz) = wed + nnz*(qnhd-qned) + (qnh-qne)*nnzd
      ww1(i, j, ivz) = we + (qnh-qne)*nnz
! correct the pressure if a k-equation is present.
      if (correctfork) then
        pp1d(i, j) = pp1d(i, j) + twothird*(ww1(i, j, itu1)*ww1d(i, j, &
&         irho)+ww1(i, j, irho)*ww1d(i, j, itu1))
...
!  bleed outflow.
    use constants
    use blockpointersiff, only : bcdata
    use bcpointers_diff, only : ww0, ww1, ww2, pp0, pp1, pp2, rlv0, rlv1&
...
&   rev0, rev0d, rev1, rev1d, rev2, rev2d, gamma2, isize, jsize, istart,&
&   jstart
    use inputphysicsiff, only : cpmodel, gammaconstant
    use utils_diff, only : terminate
    implicit none
! subroutine arguments.
    logical, intent(in) :: secondhalo, correctfork
    integer(kind=inttype), intent(in) :: nn
...
! extrapolated from the domain.
        ...
        rd = -(temp*ww2d(i, j, irho)/ww2(i, j, irho))
        r = temp
        a2d = gamma2(i, j)*(r*pp2d(i, j)+pp2(i, j)*rd)
        a2 = gamma2(i, j)*pp2(i, j)*r
        temp = sqrt(a2)
        if (a2 .eq. 0.0_8) then
          result1d = 0.0_8
        else
          result1d = a2d/(2.0*temp)
        end if
        result1 = temp
        betad = nnx*ww2d(i, j, ivx) + ww2(i, j, ivx)*nnxd + nny*ww2d(i, &
&         j, ivy) + ww2(i, j, ivy)*nnyd + nnz*ww2d(i, j, ivz) + ww2(i, j&
&         , ivz)*nnzd + two*ovgm1*result1d
...
! total temperature are prescribed.
        ...
        if (hscalinginlet) then
          temp = ww2(i, j, irhoe) + pp2(i, j)
          temp0 = htot/(r*temp)
          arg1d = (htotd-temp0*(temp*rd+r*(ww2d(i, j, irhoe)+pp2d(i, j))&
&           ))/(r*temp)
          arg1 = temp0
          temp0 = sqrt(arg1)
          if (arg1 .eq. 0.0_8) then
            ...
            scalefactd = arg1d/(2.0*temp0)
          end if
          scalefact = temp0
        else
          scalefactd = 0.0_8
        end if
        betad = scalefact*betad + beta*scalefactd
        beta = beta*scalefact
...
&         , j, ivy) + 2*ww2(i, j, ivz)*ww2d(i, j, ivz)
        ...
        temp0 = ww2(i, j, irhoe) + pp2(i, j)
        a2totd = gm1*(htotd-temp0*rd-r*(ww2d(i, j, irhoe)+pp2d(i, j))+&
&         half*q2d) + a2d
        a2tot = gm1*(htot-r*temp0+half*q2) + a2
...
! for the magnitude of the velocity.
        aa2d = half*gm1*2*alpha*alphad
        aa2 = half*gm1*alpha*alpha + one
        bbd = -(gm1*(beta*alphad+alpha*betad))
        bb = -(gm1*alpha*beta)
        ccd = half*gm1*2*beta*betad - two*ovgm1*a2totd
...
! is present, but this is normally not active.
        ddd = 2*bb*bbd - four*(cc*aa2d+aa2*ccd)
        ...
        dd = temp0
        temp0 = (dd-bb)/(two*aa2)
        qd = (ddd-bbd-temp0*two*aa2d)/(two*aa2)
        q = temp0
        if (zero .lt. q) then
          q = q
        else
...
! the gas law.
          temp0 = gamma2(i, j)*rgas
          tsd = a2d/temp0
          ts = a2/temp0
          pwx1d = (tsd-ts*ttotd/ttot)/ttot
          pwx1 = ts/ttot
          if (pwx1 .le. 0.0_8 .and. (govgm1 .eq. 0.0_8 .or. govgm1 .ne. &
&             int(govgm1))) then
            ratiod = 0.0_8
          else
          ...
          ratio = pwx1**govgm1
          pp1d(i, j) = ratio*ptotd + ptot*ratiod
          pp1(i, j) = ptot*ratio
          temp0 = ptot*ratio/(rgas*ts)
          ww1d(i, j, irho) = (ratio*ptotd+ptot*ratiod-temp0*rgas*tsd)/(&
&           rgas*ts)
          ww1(i, j, irho) = temp0
          if (correctfork) then
            pp1d(i, j) = pp1d(i, j) + twothird*(ww1(i, j, itu1)*ww1d(i, &
&             j, irho)+ww1(i, j, irho)*ww1d(i, j, itu1))
...
! determine the acoustic riemann variable that must be
! extrapolated from the domain.
        temp0 = one/ww2(i, j, irho)
        rd = -(temp0*ww2d(i, j, irho)/ww2(i, j, irho))
        r = temp0
        a2d = gamma2(i, j)*(r*pp2d(i, j)+pp2(i, j)*rd)
        a2 = gamma2(i, j)*pp2(i, j)*r
        temp0 = sqrt(a2)
...
!  correct block on the correct grid level.
    use constants
    use blockpointersiff, only : bcdata, bcdatad, addgridvelocities
    use flowvarrefstateiff, only : viscous, eddymodel, rgas, rgasd
    use inputdiscretization, only : eulerwallbctreatment
    use iteration, only : currentlevel, groundlevel
    use utils_diff, only : mydim, mydim_d
    use bcpointers_diff, only : ww0, ww0d, ww1, ww1d, ww2, ww2d, pp0, &
&   pp0d, pp1, pp1d, pp2, pp2d, pp3, pp3d, rlv0, rlv0d, rlv1, rlv1d, &
&   rlv2, rlv2d, rev0, rev0d, rev1, rev1d, rev2, rev2d, ss, ssd, ssi, &
&   ssid, ssj, ssjd, ssk, sskd, istart, isize, jstart, jsize, iend, jend
    implicit none
! subroutine arguments.
    logical, intent(in) :: secondhalo, correctfork
    integer(kind=inttype), intent(in) :: nn
! local variables.
    ...
    intrinsic max
    intrinsic min
    integer(inttype) :: max1
    integer(inttype) :: max2
    real(kind=realtype) :: temp
    real(kind=realtype) :: temp0
    real(kind=realtype) :: temp1
...
! and the normal vectors in i, j and k-direction.
        temp = bcdata(nn)%norm(j, k, 1)
        temp0 = bcdata(nn)%norm(j, k, 2)
        temp1 = bcdata(nn)%norm(j, k, 3)
        rid = sixa*bcdatad(nn)%norm(j, k, 1) + temp*sixad + siya*bcdatad&
&         (nn)%norm(j, k, 2) + temp0*siyad + siza*bcdatad(nn)%norm(j, k&
&         , 3) + temp1*sizad
        ri = temp*sixa + temp0*siya + temp1*siza
        temp1 = bcdata(nn)%norm(j, k, 1)
        temp0 = bcdata(nn)%norm(j, k, 2)
        temp = bcdata(nn)%norm(j, k, 3)
        rjd = sjxa*bcdatad(nn)%norm(j, k, 1) + temp1*sjxad + sjya*&
&         bcdatad(nn)%norm(j, k, 2) + temp0*sjyad + sjza*bcdatad(nn)%&
&         norm(j, k, 3) + temp*sjzad
        rj = temp1*sjxa + temp0*sjya + temp*sjza
        temp1 = bcdata(nn)%norm(j, k, 1)
        temp0 = bcdata(nn)%norm(j, k, 2)
        temp = bcdata(nn)%norm(j, k, 3)
        rkd = skxa*bcdatad(nn)%norm(j, k, 1) + temp1*skxad + skya*&
&         bcdatad(nn)%norm(j, k, 2) + temp0*skyad + skza*bcdatad(nn)%&
&         norm(j, k, 3) + temp*skzad
        rk = temp1*skxa + temp0*skya + temp*skza
...
! k-direction.
        qjd = sjxa*uuxd + uux*sjxad + sjya*uuyd + uuy*sjyad + sjza*uuzd &
&         + uuz*sjzad
        qj = uux*sjxa + uuy*sjya + uuz*sjza
        qkd = skxa*uuxd + uux*skxad + skya*uuyd + uuy*skyad + skza*uuzd &
&         + uuz*skzad
...
! be that an additional term is needed there.
        temp1 = uux*rxj + uuy*ryj + uuz*rzj
        temp0 = uux*rxk + uuy*ryk + uuz*rzk
        temp = qj*temp1 + qk*temp0
        temp2 = (temp*ww2(j, k, irho)-rj*dpj-rk*dpk)/ri
        gradd(j, k) = (ww2(j, k, irho)*(temp1*qjd+qj*(rxj*uuxd+uux*rxjd+&
&         ryj*uuyd+uuy*ryjd+rzj*uuzd+uuz*rzjd)+temp0*qkd+qk*(rxk*uuxd+&
&         uux*rxkd+ryk*uuyd+uuy*rykd+rzk*uuzd+uuz*rzkd))+temp*ww2d(j, k&
&         , irho)-dpj*rjd-rj*dpjd-dpk*rkd-rk*dpkd-temp2*rid)/ri
        grad(j, k) = temp2
      end do
    case default
      gradd = 0.0_8
...
&       ), pp1(j, k))
      temp2 = bcdata(nn)%norm(j, k, 1)
      temp1 = bcdata(nn)%norm(j, k, 2)
      temp0 = bcdata(nn)%norm(j, k, 3)
      vnd = two*(-(temp2*ww2d(j, k, ivx))-ww2(j, k, ivx)*bcdatad(nn)%&
&       norm(j, k, 1)-temp1*ww2d(j, k, ivy)-ww2(j, k, ivy)*bcdatad(nn)%&
&       norm(j, k, 2)-temp0*ww2d(j, k, ivz)-ww2(j, k, ivz)*bcdatad(nn)%&
&       norm(j, k, 3))
      vn = two*(bcdata(nn)%rface(j, k)-ww2(j, k, ivx)*temp2-ww2(j, k, &
&       ivy)*temp1-ww2(j, k, ivz)*temp0)
      ww1d(j, k, irho) = ww2d(j, k, irho)
      ww1(j, k, irho) = ww2(j, k, irho)
      temp2 = bcdata(nn)%norm(j, k, 1)
      ww1d(j, k, ivx) = ww2d(j, k, ivx) + temp2*vnd + vn*bcdatad(nn)%&
&       norm(j, k, 1)
      ww1(j, k, ivx) = ww2(j, k, ivx) + vn*temp2
      temp2 = bcdata(nn)%norm(j, k, 2)
      ww1d(j, k, ivy) = ww2d(j, k, ivy) + temp2*vnd + vn*bcdatad(nn)%&
&       norm(j, k, 2)
      ww1(j, k, ivy) = ww2(j, k, ivy) + vn*temp2
      temp2 = bcdata(nn)%norm(j, k, 3)
      ww1d(j, k, ivz) = ww2d(j, k, ivz) + temp2*vnd + vn*bcdatad(nn)%&
&       norm(j, k, 3)
      ww1(j, k, ivz) = ww2(j, k, ivz) + vn*temp2
...
!  correct block on the correct grid level.
    use constants
    use blockpointersiff, only : bcdata, addgridvelocities
    use flowvarrefstateiff, only : viscous, eddymodel, rgas
    use inputdiscretization, only : eulerwallbctreatment
    use iteration, only : currentlevel, groundlevel
    use utils_diff, only : mydim
    use bcpointers_diff, only : ww0, ww1, ww2, pp0, pp1, pp2, pp3, rlv0,&
...
! it is assumed that the bcpointers are already set *
    use constants
    use blockpointersiff, only : bcdata, bcdatad
    use flowvarrefstateiff, only : eddymodel, viscous, gammainf, winf,&
&   winfd, pinfcorr, pinfcorrd
    use bcpointers_diff, only : ww0, ww0d, ww1, ww1d, ww2, ww2d, pp0, &
&   pp0d, pp1, pp1d, pp2, pp2d, rlv0, rlv0d, rlv1, rlv1d, rlv2, rlv2d, &
&   rev0, rev0d, rev1, rev1d, rev2, rev2d, gamma2, istart, jstart, isize&
&   , jsize
    implicit none
! subroutine arguments.
    logical, intent(in) :: secondhalo, correctfork
...
! the entropy of the free stream.
    ...
    w0d = winfd(ivz)
    w0 = winf(ivz)
    arg1d = gammainf*(r0*pinfcorrd+pinfcorr*r0d)
    ...
    c0 = temp
    if (winf(irho) .le. 0.0_8 .and. (gammainf .eq. 0.0_8 .or. gammainf &
&       .ne. int(gammainf))) then
    ...
    s0d = (pwr1d-pwr1*pinfcorrd/pinfcorr)/pinfcorr
    s0 = pwr1/pinfcorr
! loop over the generic subface to set the state in the
! halo cells.
    do ii=0,isize*jsize-1
...
! substract the normal velocity of the mesh.
      ...
      temp0 = bcdata(nn)%norm(i, j, 2)
      temp1 = bcdata(nn)%norm(i, j, 3)
      qn0d = temp*u0d + u0*bcdatad(nn)%norm(i, j, 1) + temp0*v0d + v0*&
&       bcdatad(nn)%norm(i, j, 2) + temp1*w0d + w0*bcdatad(nn)%norm(i, j&
&       , 3)
      qn0 = u0*temp + v0*temp0 + w0*temp1
...
! in the internal cell.
      temp1 = one/ww2(i, j, irho)
      red = -(temp1*ww2d(i, j, irho)/ww2(i, j, irho))
      re = temp1
      ued = ww2d(i, j, ivx)
      ue = ww2(i, j, ivx)
      ved = ww2d(i, j, ivy)
      ...
      temp1 = bcdata(nn)%norm(i, j, 1)
      temp0 = bcdata(nn)%norm(i, j, 2)
      temp = bcdata(nn)%norm(i, j, 3)
      qned = temp1*ued + ue*bcdatad(nn)%norm(i, j, 1) + temp0*ved + ve*&
&       bcdatad(nn)%norm(i, j, 2) + temp*wed + we*bcdatad(nn)%norm(i, j&
&       , 3)
      qne = ue*temp1 + ve*temp0 + we*temp
      arg1d = gamma2(i, j)*(re*pp2d(i, j)+pp2(i, j)*red)
...
! outflow.
        temp1 = bcdata(nn)%norm(i, j, 1)
        ufd = ued + temp1*(qnfd-qned) + (qnf-qne)*bcdatad(nn)%norm(i, j&
&         , 1)
        uf = ue + (qnf-qne)*temp1
        temp1 = bcdata(nn)%norm(i, j, 2)
        vfd = ved + temp1*(qnfd-qned) + (qnf-qne)*bcdatad(nn)%norm(i, j&
&         , 2)
        vf = ve + (qnf-qne)*temp1
        temp1 = bcdata(nn)%norm(i, j, 3)
        wfd = wed + temp1*(qnfd-qned) + (qnf-qne)*bcdatad(nn)%norm(i, j&
&         , 3)
        wf = we + (qnf-qne)*temp1
        if (ww2(i, j, irho) .le. 0.0_8 .and. (gamma2(i, j) .eq. 0.0_8 &
&           .or. gamma2(i, j) .ne. int(gamma2(i, j)))) then
          pwr1d = 0.0_8
        else
...
&           , j, irho)
        ...
        temp1 = pwr1/pp2(i, j)
        sfd = (pwr1d-temp1*pp2d(i, j))/pp2(i, j)
        sf = temp1
      else
! inflow
        temp1 = bcdata(nn)%norm(i, j, 1)
        ufd = u0d + temp1*(qnfd-qn0d) + (qnf-qn0)*bcdatad(nn)%norm(i, j&
&         , 1)
        uf = u0 + (qnf-qn0)*temp1
        temp1 = bcdata(nn)%norm(i, j, 2)
        vfd = v0d + temp1*(qnfd-qn0d) + (qnf-qn0)*bcdatad(nn)%norm(i, j&
&         , 2)
        vf = v0 + (qnf-qn0)*temp1
        temp1 = bcdata(nn)%norm(i, j, 3)
        wfd = w0d + temp1*(qnfd-qn0d) + (qnf-qn0)*bcdatad(nn)%norm(i, j&
&         , 3)
        wf = w0 + (qnf-qn0)*temp1
        sfd = s0d
        sf = s0
...
! halo cell.
      ccd = 2*cf*cfd/gamma2(i, j)
      cc = cf*cf/gamma2(i, j)
      qq = uf*uf + vf*vf + wf*wf
      pwx1d = cc*sfd + sf*ccd
      pwx1 = sf*cc
      if (pwx1 .le. 0.0_8 .and. (ovgm1 .eq. 0.0_8 .or. ovgm1 .ne. int(&
&         ovgm1))) then
        ww1d(i, j, irho) = 0.0_8
      else
      ...
      ww1d(i, j, ivz) = wfd
      ww1(i, j, ivz) = wf
      pp1d(i, j) = cc*ww1d(i, j, irho) + ww1(i, j, irho)*ccd
...
! the correct block on the correct grid level.
    ...
    use blockpointersiff, only : bcdata
    use flowvarrefstateiff, only : eddymodel, viscous
    use bcpointers_diff, only : ww0, ww1, pp0, pp1, rlv0, rlv1, rlv2, &
...
!use inputphysics
    use bcpointers_diff, only : ww1, ww2, ww3, pp1, pp2, pp3, rlv1, rlv2&
...
!  currently point.
    use constants
    use cpcurvefits
    use flowvarrefstateiff, only : rgas, tref
    use inputphysicsiff, only : cpmodel, gammaconstant
    use bcpointers_diff, only : isize, jsize, istart, jstart
    implicit none
! local parameter.
    real(kind=realtype), parameter :: twothird=two*third
! subroutine arguments.
...
! only implements the constant cpmodel
    use constants
    use inputphysicsiff, only : gammaconstant, cpmodel
    use utils_diff, only : terminate
    use bcpointers_diff, only : isize, jsize, istart, jstart
    implicit none
    real(kind=realtype), dimension(:, :) :: pp
...
! correcting for k
      do ii=0,isize*jsize-1
        ...
        if (.not.correctfork) then
          temp = ww(i, j, ivx)*ww(i, j, ivx) + ww(i, j, ivy)*ww(i, j, &
&           ivy) + ww(i, j, ivz)*ww(i, j, ivz)
          wwd(i, j, irhoe) = ovgm1*ppd(i, j) + half*(temp*wwd(i, j, irho&
&           )+ww(i, j, irho)*(2*ww(i, j, ivx)*wwd(i, j, ivx)+2*ww(i, j, &
&           ivy)*wwd(i, j, ivy)+2*ww(i, j, ivz)*wwd(i, j, ivz)))
          ww(i, j, irhoe) = ovgm1*pp(i, j) + half*(ww(i, j, irho)*temp)
        else
          temp = ww(i, j, ivx)*ww(i, j, ivx) + ww(i, j, ivy)*ww(i, j, &
&           ivy) + ww(i, j, ivz)*ww(i, j, ivz)
          wwd(i, j, irhoe) = ovgm1*ppd(i, j) + half*(temp*wwd(i, j, irho&
&           )+ww(i, j, irho)*(2*ww(i, j, ivx)*wwd(i, j, ivx)+2*ww(i, j, &
&           ivy)*wwd(i, j, ivy)+2*ww(i, j, ivz)*wwd(i, j, ivz))) - factk&
&           *(ww(i, j, itu1)*wwd(i, j, irho)+ww(i, j, irho)*wwd(i, j, &
&           itu1))
          ww(i, j, irhoe) = ovgm1*pp(i, j) + half*(ww(i, j, irho)*temp) &
...
! that the appropriate bcpointers are already set
    ...
    use bcpointers_diff, only : ww0, ww0d, ww1, ww1d, ww2, ww2d, pp0, &
&   pp0d, pp1, pp1d, pp2, pp2d, rlv0, rlv0d, rlv1, rlv1d, rlv2, rlv2d, &
&   rev0, rev0d, rev1, rev1d, rev2, rev2d, isize, jsize, istart, jstart
    use flowvarrefstateiff, only : viscous, eddymodel
    implicit none
! input variables
    logical, intent(in) :: correctfork
