...
!                pinfcorr:out pref:out pinfdim:in muinfdim:(loc)
!                rgas:out rhoref:out
...
!
    use constants
    use paramturbiff
    use inputphysicsiff, only : equations, mach, machd, machcoef, &
...
&   rgasdim, ssuthdim, eddyvisinfratio, turbmodel, turbintensityinf
    use flowvarrefstateiff, only : pinfdim, pinfdimd, tinfdim, &
&   tinfdimd, rhoinfdim, rhoinfdimd, muinfdim, muinfdimd, pref, prefd, &
&   rhoref, rhorefd, tref, trefd, muref, murefd, timeref, timerefd, uref&
&   , urefd, href, hrefd, pinf, pinfd, pinfcorr, pinfcorrd, rhoinf, &
&   rhoinfd, uinf, uinfd, rgas, rgasd, muinf, muinfd, gammainf, winf, &
&   winfd, nw, nwf, kpresent, winf, winfd
    use flowutils_diff, only : computegamma, etot, etot_d
    use turbutils_diff, only : sanuknowneddyratio, sanuknowneddyratio_d
    implicit none
...
! meters.
    temp0 = sqrt(pref*rhoref)
    if (pref*rhoref .eq. 0.0_8) then
      murefd = 0.0_8
    else
      murefd = (rhoref*prefd+pref*rhorefd)/(2.0*temp0)
    end if
    muref = temp0
...
! concerning the reference length.
    ...
    if (temp0 .eq. 0.0_8) then
      ...
      timerefd = (rhorefd-temp0*prefd)/(2.0*temp1*pref)
    end if
    timeref = temp1
    href = pref/rhoref
    uref = sqrt(href)
...
! viscosity and gas constant.
    ...
    uinf = mach*result1
    temp1 = rhoref*tref/pref
    rgasd = rgasdim*(tref*rhorefd+rhoref*trefd-temp1*prefd)/pref
    rgas = rgasdim*temp1
...
! geometries. (not used in sa model)
    temp1 = pinf/rhoinf
    uinf2d = gammainf*(temp1*2*machcoef*machcoefd+machcoef**2*(pinfd-&
&     temp1*rhoinfd)/rhoinf)
    uinf2 = gammainf*(machcoef*machcoef*temp1)
...
!=============================================================
        ...
        temp1 = winf(itu1)/(eddyvisinfratio*nuinf)
        winfd(itu2) = (winfd(itu1)-temp1*eddyvisinfratio*nuinfd)/(&
...
!=============================================================
        winfd(itu1) = turbintensityinf**2*1.5_realtype*uinf2d
        winf(itu1) = 1.5_realtype*uinf2*turbintensityinf**2
        temp1 = nuinf/winf(itu1)
        winfd(itu2) = eddyvisinfratio*(nuinfd-temp1*winfd(itu1))/winf(&
&         itu1)
        winf(itu2) = eddyvisinfratio*temp1
      case (v2f) 
!=============================================================
        winfd(itu1) = turbintensityinf**2*1.5_realtype*uinf2d
        winf(itu1) = 1.5_realtype*uinf2*turbintensityinf**2
        temp1 = winf(itu1)*winf(itu1)/(eddyvisinfratio*nuinf)
        winfd(itu2) = 0.09_realtype*(2*winf(itu1)*winfd(itu1)-temp1*&
&         eddyvisinfratio*nuinfd)/(eddyvisinfratio*nuinf)
        winf(itu2) = 0.09_realtype*temp1
...
!
    use constants
    use paramturbiff
    use inputphysicsiff, only : equations, mach, machcoef, musuthdim, &
&   tsuthdim, veldirfreestream, rgasdim, ssuthdim, eddyvisinfratio, &
&   turbmodel, turbintensityinf
    use flowvarrefstateiff, only : pinfdim, tinfdim, rhoinfdim, &
&   muinfdim, pref, rhoref, tref, muref, timeref, uref, href, pinf, &
&   pinfcorr, rhoinf, uinf, rgas, muinf, gammainf, winf, nw, nwf, &
&   kpresent, winf
    use flowutils_diff, only : computegamma, etot
    use turbutils_diff, only : sanuknowneddyratio
    implicit none
    integer(kind=inttype) :: sps, nn, mm, ierr
    real(kind=realtype) :: gm1, ratio
    real(kind=realtype) :: nuinf, ktmp, uinf2
...
end module initializeflow_diff
