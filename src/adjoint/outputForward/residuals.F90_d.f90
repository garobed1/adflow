...
! added by hdn
    ...
    use iteration
    use inputadjoint
    use flowutils_diff, only : computespeedofsoundsquared, &
&   allnodalgradients
    use fluxes_diff
    implicit none
!
...
!   with respect to varying inputs: uref pref *w *dw actuatorregions.f
!                plocal
...
! block pointers are already set.
    use constants
    use actuatorregiondataiff
    use blockpointersiff, only : volref, dw, dwd, w, wd
    use flowvarrefstateiff, only : pref, prefd, uref, urefd
    use communication
...
! compute the constant force factor
    ...
    factd = factor*(actuatorregionsd(iregion)%f-actuatorregions(iregion)&
&     %f*actuatorregions(iregion)%volume*prefd/temp)/temp
    fact = factor*(actuatorregions(iregion)%f/temp)
...
! add in the local power contribution:
        temp = vx*ftmp(1) + vy*ftmp(2) + vz*ftmp(3)
        plocald = plocald + redim*(ftmp(1)*vxd+vx*ftmpd(1)+ftmp(2)*vyd+&
&         vy*ftmpd(2)+ftmp(3)*vzd+vz*ftmpd(3)) + temp*redimd
        plocal = plocal + temp*redim
      end if
    end do
...
! block pointers are already set.
    use constants
    use actuatorregiondataiff
    use blockpointersiff, only : volref, dw, w
    use flowvarrefstateiff, only : pref, uref
    use communication
    use iteration, only : ordersconverged
    implicit none
! input
