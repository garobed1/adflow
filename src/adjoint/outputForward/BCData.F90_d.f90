...
! bcdata.
  ...
  subroutine setbcvarnamessubsonicinflow()
    ...
    use flowvarrefstateiff, only : nwt
    implicit none
!
!      local variables.
!
...
!
    use constants
    use cgnsnames
    use inputphysicsiff, only : equations, turbmodel
    implicit none
!
...
!
    ...
    use inputphysicsiff, only : cpmodel, gammaconstant, rgasdim
    use flowvarrefstateiff, only : pinfdim, pinfdimd
    implicit none
!
!      subroutine arguments.
!
...
!
    ...
    use communication, only : myid
    use inputphysicsiff, only : cpmodel, gammaconstant, rgasdim
    use flowvarrefstateiff, only : pinfdim
    implicit none
!
!      subroutine arguments.
!
...
!
    use constants
    use blockpointersiff, only : bcfaceid, bcdata, x, si, sj, sk, il, &
&   jl, kl, sectionid
...
!
    use constants
    use cgnsnames
    use blockpointersiff, only : bcfaceid, bcdata, bcdatad, nbkglobal
    use utils_diff, only : terminate, sitemperature
    use flowvarrefstateiff, only : tref, trefd
    use diffsizes
...
! convert to si-units and store the temperature in tns_wall.
    ...
    do j=jbeg,jend
      do i=ibeg,iend
        temp0 = (trans+mult*bcvararray(i, j, 1))/tref
        bcdatad(boco)%tns_wall(i, j) = (mult*bcvararrayd(i, j, 1)-temp0*&
&         trefd)/tref
...
!
    use constants
    use cgnsnames
    use blockpointersiff, only : bcfaceid, bcdata, nbkglobal
    use utils_diff, only : terminate, sitemperature
    use flowvarrefstateiff, only : tref
    implicit none
!
!      subroutine arguments.
!
...
!
    use constants
    use cgnsnames
    use blockpointersiff, only : nbkglobal, sectionid, bcfaceid, &
&   bcdata, bcdatad
    use flowvarrefstateiff, only : tref, trefd, pref, prefd, href, &
&   hrefd, rhoref, rhorefd, muref, murefd, nwt, winf, winfd
    use inputphysicsiff, only : equations
    use utils_diff, only : sidensity, sivelocity, sipressure, siangle, &
...
!
      ...
      use inputphysicsiff, only : rgasdim
      use section, only : sections
      use diffsizes
!  hint: isize1ofdrfbcdata should be the size of dimension 1 of array *bcdata
      implicit none
...
! store it.
      if (ptpresent) then
        ...
        do j=jbeg,jend
          do i=ibeg,iend
            temp0 = (trans+mult*bcvararray(i, j, 1))/pref
            bcdatad(boco)%ptinlet(i, j) = (mult*bcvararrayd(i, j, 1)-&
&             temp0*prefd)/pref
            bcdata(boco)%ptinlet(i, j) = temp0
          end do
        end do
      else if (associated(bcdatad)) then
        do ii1=1,isize1ofdrfbcdata
          bcdatad(ii1)%ptinlet = 0.0_8
        end do
      end if
...
! and store it.
      if (ttpresent) then
        ...
        do j=jbeg,jend
          do i=ibeg,iend
            temp0 = (trans+mult*bcvararray(i, j, 2))/tref
            bcdatad(boco)%ttinlet(i, j) = (mult*bcvararrayd(i, j, 2)-&
&             temp0*trefd)/tref
            bcdata(boco)%ttinlet(i, j) = temp0
          end do
        end do
      else if (associated(bcdatad)) then
        do ii1=1,isize1ofdrfbcdata
...
! gas law to obtain the total temperature.
          do j=jbeg,jend
            do i=ibeg,iend
              ...
              temp2 = temp1*pref/temp0
              bcdatad(boco)%ttinlet(i, j) = (pref*bcdatad(boco)%ptinlet(&
&               i, j)+temp1*prefd-temp2*(tref*rgasdim*rhotd+rgasdim*rhot&
&               *trefd))/temp0
              bcdata(boco)%ttinlet(i, j) = temp2
            end do
          end do
...
! gas law to obtain the total pressure.
          do j=jbeg,jend
            do i=ibeg,iend
              ...
              temp1 = bcdata(boco)%ttinlet(i, j)
              bcdatad(boco)%ptinlet(i, j) = rgasdim*(temp2*bcdatad(boco)&
&               %ttinlet(i, j)+temp1*(tref*rhotd+rhot*trefd-temp2*prefd)&
&               /pref)
              bcdata(boco)%ptinlet(i, j) = rgasdim*(temp1*temp2)
            end do
          end do
        end if
      end if
...
! total temperature.
          temp2 = bcdata(boco)%ttinlet(i, j)
          tdimd = tref*bcdatad(boco)%ttinlet(i, j) + temp2*trefd
          tdim = temp2*tref
          call computehtot_d(tdim, tdimd, hdim, hdimd)
          bcdatad(boco)%htinlet(i, j) = (hdimd-hdim*hrefd/href)/href
...
!
    ...
    use blockpointersiff, only : nbkglobal, sectionid, bcfaceid, &
&   bcdata
    use flowvarrefstateiff, only : tref, pref, href, rhoref, muref, &
&   nwt, winf
    use inputphysicsiff, only : equations
    use utils_diff, only : sidensity, sivelocity, sipressure, siangle, &
&   sitemperature, terminate
...
!
    use constants
    use cgnsnames
    use blockpointersiff, only : bcdata, bcdatad, nbkglobal, bcfaceid
    use utils_diff, only : terminate, sipressure
    use flowvarrefstateiff, only : pref, prefd
    use diffsizes
!  hint: isize1ofdrfbcdata should be the size of dimension 1 of array *bcdata
    implicit none
!
...
! convert to si-units and store the pressure in ps.
    ...
    do j=jbeg,jend
      do i=ibeg,iend
        temp0 = (trans+mult*bcvararray(i, j, 1))/pref
        bcdatad(boco)%ps(i, j) = (mult*bcvararrayd(i, j, 1)-temp0*prefd)&
&         /pref
        bcdata(boco)%ps(i, j) = temp0
      end do
    end do
 100 format('zone ',a,', boundary subface ',a, &
...
!
    use constants
    use cgnsnames
    use blockpointersiff, only : bcdata, nbkglobal, bcfaceid
    use utils_diff, only : terminate, sipressure
    use flowvarrefstateiff, only : pref
    implicit none
!
!      subroutine arguments.
!
...
!
    ...
    use blockpointersiff, only : bcdata, bcdatad, nbkglobal, bcfaceid,&
&   sectionid
    use flowvarrefstateiff, only : nwt, pinfcorr, pinfcorrd, winf, &
&   winfd, uref, urefd, rhoref, rhorefd, pref, prefd, muref, murefd
    use inputphysicsiff, only : equations, flowtype, veldirfreestream,&
&   veldirfreestreamd
    use utils_diff, only : sidensity, sipressure, sivelocity, &
&   sitemperature, terminate
    use diffsizes
!  hint: isize1ofdrfbcdata should be the size of dimension 1 of array *bcdata
    implicit none
...
! into account.
      ...
      do j=jbeg,jend
        do i=ibeg,iend
          temp0 = (trans+mult*bcvararray(i, j, 1))/rhoref
          bcdatad(boco)%rho(i, j) = (mult*bcvararrayd(i, j, 1)-temp0*&
&           rhorefd)/rhoref
          bcdata(boco)%rho(i, j) = temp0
        end do
      end do
...
! into account.
      ...
      do j=jbeg,jend
        do i=ibeg,iend
          temp0 = (trans+mult*bcvararray(i, j, 2))/pref
          bcdatad(boco)%ps(i, j) = (mult*bcvararrayd(i, j, 2)-temp0*&
&           prefd)/pref
          bcdata(boco)%ps(i, j) = temp0
        end do
      end do
...
! store the values.
            temp0 = (axis(1)*vloc(1)+radvec1(1)*vloc(2)+radvec2(1)*vloc(&
&             3))/uref
            bcdatad(boco)%velx(i, j) = (axis(1)*vlocd(1)+radvec1(1)*&
&             vlocd(2)+radvec2(1)*vlocd(3)-temp0*urefd)/uref
            bcdata(boco)%velx(i, j) = temp0
            temp0 = (axis(2)*vloc(1)+radvec1(2)*vloc(2)+radvec2(2)*vloc(&
&             3))/uref
            bcdatad(boco)%vely(i, j) = (axis(2)*vlocd(1)+radvec1(2)*&
&             vlocd(2)+radvec2(2)*vlocd(3)-temp0*urefd)/uref
            bcdata(boco)%vely(i, j) = temp0
            temp0 = (axis(3)*vloc(1)+radvec1(3)*vloc(2)+radvec2(3)*vloc(&
&             3))/uref
            bcdatad(boco)%velz(i, j) = (axis(3)*vlocd(1)+radvec1(3)*&
&             vlocd(2)+radvec2(3)*vlocd(3)-temp0*urefd)/uref
            bcdata(boco)%velz(i, j) = temp0
          end do
        end do
      else
...
! set the velocities.
        do j=jbeg,jend
          do i=ibeg,iend
            temp0 = (transvel(1)+multvel(1)*bcvararray(i, j, 3))/uref
            bcdatad(boco)%velx(i, j) = (multvel(1)*bcvararrayd(i, j, 3)-&
&             temp0*urefd)/uref
            bcdata(boco)%velx(i, j) = temp0
            temp0 = (transvel(2)+multvel(2)*bcvararray(i, j, 4))/uref
            bcdatad(boco)%vely(i, j) = (multvel(2)*bcvararrayd(i, j, 4)-&
&             temp0*urefd)/uref
            bcdata(boco)%vely(i, j) = temp0
            temp0 = (transvel(3)+multvel(3)*bcvararray(i, j, 5))/uref
...
!
    ...
    use blockpointersiff, only : bcdata, nbkglobal, bcfaceid, &
&   sectionid
    use flowvarrefstateiff, only : nwt, pinfcorr, winf, uref, rhoref, &
&   pref, muref
    use inputphysicsiff, only : equations, flowtype, veldirfreestream
    use utils_diff, only : sidensity, sipressure, sivelocity, &
&   sitemperature, terminate
...
!
    use constants
    use flowvarrefstateiff, only : nt1, nt2, muref, murefd, pref, &
&   prefd, rhoref, rhorefd, winf, winfd
    use inputphysicsiff, only : equations, turbmodel
    use utils_diff, only : terminate, siturb
    implicit none
!
!      subroutine arguments.
!
...
! if not the rans equations are solved.
    setbcvarturb = .true.
    if (equations .ne. ransequations) then
      if (associated(turbinletd)) turbinletd = 0.0_8
      return
    else
! set the reference values depending on the turbulence model.
      nurefd = (murefd-muref*rhorefd/rhoref)/rhoref
      nuref = muref/rhoref
      select case  (turbmodel) 
      case (spalartallmaras, spalartallmarasedwards) 
        refd = 0.0_8
      ...
      case (ktau) 
        ...
        refd(itu1) = (prefd-pref*rhorefd/rhoref)/rhoref
        ref(itu1) = pref/rhoref
        temp0 = nuref/ref(itu1)
        refd(itu2) = (nurefd-temp0*refd(itu1))/ref(itu1)
        ref(itu2) = temp0
      case (v2f) 
        refd = 0.0_8
        refd(itu1) = (prefd-pref*rhorefd/rhoref)/rhoref
        ref(itu1) = pref/rhoref
        temp0 = ref(itu1)/nuref
        refd(itu4) = (refd(itu1)-temp0*nurefd)/nuref
        ref(itu4) = temp0
        refd(itu2) = ref(itu4)*refd(itu1) + ref(itu1)*refd(itu4)
...
! set the turbulent variables.
          do j=jbeg,jend
            do i=ibeg,iend
              temp0 = (trans+mult*bcvararray(i, j, mm))/ref(nn)
              turbinletd(i, j, nn) = (mult*bcvararrayd(i, j, mm)-temp0*&
&               refd(nn))/ref(nn)
              turbinlet(i, j, nn) = temp0
            end do
          end do
        else
...
!
    use constants
    use flowvarrefstateiff, only : nt1, nt2, muref, pref, rhoref, winf
    use inputphysicsiff, only : equations, turbmodel
    use utils_diff, only : terminate, siturb
    implicit none
!
