...
!   with respect to varying inputs: timeref *p *w *dw *vol *si
!                *sj *sk
!   rw status of diff variables: timeref:in *p:in *w:in *dw:in-out
!                *vol:in *si:in *sj:in *sk:in
!   plus diff mem management of: p:in w:in dw:in vol:in si:in sj:in
!                sk:in
...
!
    use constants
    use blockpointersiff, only : nx, il, ie, ny, jl, je, nz, kl, ke, &
...
&   factfamilyi, factfamilyj, factfamilyk
    ...
    use flowvarrefstateiff, only : timeref, timerefd
    use inputphysicsiff, only : equationmode
    implicit none
!
!      local variables.
!
...
&   factfamilyk
    use cgnsgrid, only : cgnsdoms, massflowfamilyinv
    use flowvarrefstateiff, only : timeref
    use inputphysicsiff, only : equationmode
    implicit none
!
!      local variables.
...
!
    use constants
    use blockpointersiff, only : nx, ny, nz, il, jl, kl, ie, je, ke, &
&   ib, jb, kb, w, wd, p, pd, pori, porj, pork, fw, fwd, gamma, si, sid,&
&   sj, sjd, sk, skd, indfamilyi, indfamilyj, indfamilyk, spectralsol, &
&   addgridvelocities, sfacei, sfacej, sfacek, factfamilyi, factfamilyj,&
&   factfamilyk
    use flowvarrefstateiff, only : pinfcorr, pinfcorrd
    use inputdiscretization, only : vis2, vis4
    use inputphysicsiff, only : equations
    use iteration, only : rfil
    use cgnsgrid, only : massflowfamilydiss
    use utils_diff, only : getcorrectfork, mydim, mydim_d
    implicit none
!
!      local parameters.
!
...
&   sfacei, sfacej, sfacek, factfamilyi, factfamilyj, factfamilyk
    use flowvarrefstateiff, only : pinfcorr
    use inputdiscretization, only : vis2, vis4
    use inputphysicsiff, only : equations
    use iteration, only : rfil
    use cgnsgrid, only : massflowfamilydiss
    use utils_diff, only : getcorrectfork, mydim
    implicit none
!
!      local parameters.
!
...
&   radj, radjd, radk, radkd, gamma
    use flowvarrefstateiff, only : gammainf, pinfcorr, pinfcorrd, &
&   rhoinf, rhoinfd
    use inputdiscretization, only : vis2, vis4
    use inputphysicsiff, only : equations
    use iteration, only : rfil
...
&           .ne. int(gammainf))) then
        ...
        sslimd = 0.001_realtype*(pinfcorrd-pinfcorr*pwr1d/pwr1)/pwr1
...
! store the entropy in ss. see above.
        do k=0,kb
          do j=0,jb
            do i=0,ib
              if (w(i, j, k, irho) .le. 0.0_8 .and. (gamma(i, j, k) .eq.&
&                 0.0_8 .or. gamma(i, j, k) .ne. int(gamma(i, j, k)))) &
&             then
                pwr1d = 0.0_8
              else
...
&                 -1)*wd(i, j, k, irho)
              ...
              temp = p(i, j, k)/pwr1
              ssd(i, j, k) = (pd(i, j, k)-temp*pwr1d)/pwr1
              ss(i, j, k) = temp
            end do
          end do
        end do
...
! compute the pressure sensor for each cell, in each direction:
      do k=1,ke
        do j=1,je
          do i=1,ie
            temp = ss(i+1, j, k) + two*ss(i, j, k) + ss(i-1, j, k) + &
&             sslim
            temp0 = (ss(i+1, j, k)-two*ss(i, j, k)+ss(i-1, j, k))/temp
            x1d = (ssd(i+1, j, k)-two*ssd(i, j, k)+ssd(i-1, j, k)-temp0*&
...
&             temp
            ...
            temp0 = ss(i, j+1, k) + two*ss(i, j, k) + ss(i, j-1, k) + &
&             sslim
            temp = (ss(i, j+1, k)-two*ss(i, j, k)+ss(i, j-1, k))/temp0
            x2d = (ssd(i, j+1, k)-two*ssd(i, j, k)+ssd(i, j-1, k)-temp*(&
...
&             temp0
            ...
            temp0 = ss(i, j, k+1) + two*ss(i, j, k) + ss(i, j, k-1) + &
&             sslim
            temp = (ss(i, j, k+1)-two*ss(i, j, k)+ss(i, j, k-1))/temp0
            x3d = (ssd(i, j, k+1)-two*ssd(i, j, k)+ssd(i, j, k-1)-temp*(&
...
! appropriate sliding mesh interface.
            ...
            ddw1 = w(i+1, j, k, irho) - w(i, j, k, irho)
            temp0 = w(i+2, j, k, irho) - w(i-1, j, k, irho) - three*ddw1
            fsd = ddw1*dis2d + dis2*ddw1d - temp0*dis4d - dis4*(wd(i+2, &
&             j, k, irho)-wd(i-1, j, k, irho)-three*ddw1d)
            fs = dis2*ddw1 - dis4*temp0
            fwd(i+1, j, k, irho) = fwd(i+1, j, k, irho) + fsd
            fw(i+1, j, k, irho) = fw(i+1, j, k, irho) + fs
            fwd(i, j, k, irho) = fwd(i, j, k, irho) - fsd
            fw(i, j, k, irho) = fw(i, j, k, irho) - fs
! x-momentum.
            ...
            temp1 = w(i, j, k, irho)
            temp2 = w(i, j, k, ivx)
            ddw2d = temp0*wd(i+1, j, k, ivx) + temp*wd(i+1, j, k, irho) &
&             - temp1*wd(i, j, k, ivx) - temp2*wd(i, j, k, irho)
            ddw2 = temp*temp0 - temp2*temp1
            temp2 = w(i-1, j, k, irho)
            temp1 = w(i-1, j, k, ivx)
            temp0 = w(i+2, j, k, irho)
...
&   ib, jb, kb, w, p, pori, porj, pork, fw, radi, radj, radk, gamma
    use flowvarrefstateiff, only : gammainf, pinfcorr, rhoinf
    use inputdiscretization, only : vis2, vis4
    use inputphysicsiff, only : equations
    use iteration, only : rfil
...
!
    use constants
    use blockpointersiff, only : il, jl, kl, ie, je, ke, ib, jb, kb, w&
...
&   rotmatrixk, factfamilyi, factfamilyj, factfamilyk
    use flowvarrefstateiff, only : kpresent, nw, nwf, rgas, rgasd, &
&   tref, trefd
...
&   riemann, riemanncoarse, orderturb, kappacoef
    ...
    use cgnsgrid, only : massflowfamilydiss
    use utils_diff, only : getcorrectfork, terminate
    use flowutils_diff, only : etot
    implicit none
!
...
!
    use constants
    use blockpointersiff
    use flowvarrefstateiff
    use inputphysicsiff
    use iteration
    use diffsizes
...
! and a return can be made.
    ...
    if (abs0 .lt. thresholdreal) then
      if (associated(viscsubfaced)) then
...
!   variations   of useful results: *fw
!   with respect to varying inputs: *rev *aa *w *x *rlv *fw
...
! compute the viscous fluxes for this i-face.
          ...
          temp0 = si(i, j, k, 2)
          temp1 = si(i, j, k, 3)
          fmxd = temp*tauxxd + temp0*tauxyd + temp1*tauxzd
          fmx = temp*tauxx + temp0*tauxy + temp1*tauxz
          temp1 = si(i, j, k, 1)
          temp0 = si(i, j, k, 2)
          temp = si(i, j, k, 3)
          fmyd = temp1*tauxyd + temp0*tauyyd + temp*tauyzd
          fmy = temp1*tauxy + temp0*tauyy + temp*tauyz
          temp1 = si(i, j, k, 1)
          temp0 = si(i, j, k, 2)
          temp = si(i, j, k, 3)
          fmzd = temp1*tauxzd + temp0*tauyzd + temp*tauzzd
          fmz = temp1*tauxz + temp0*tauyz + temp*tauzz
          temp1 = si(i, j, k, 1)
          temp0 = si(i, j, k, 2)
          temp = si(i, j, k, 3)
          temp2 = si(i, j, k, 1)
          ...
          temp4 = si(i, j, k, 3)
          frhoed = temp1*(tauxx*ubard+ubar*tauxxd+tauxy*vbard+vbar*&
&           tauxyd+tauxz*wbard+wbar*tauxzd) + temp0*(tauxy*ubard+ubar*&
&           tauxyd+tauyy*vbard+vbar*tauyyd+tauyz*wbard+wbar*tauyzd) + &
&           temp*(tauxz*ubard+ubar*tauxzd+tauyz*vbard+vbar*tauyzd+tauzz*&
&           wbard+wbar*tauzzd) - temp2*q_xd - temp3*q_yd - temp4*q_zd
          frhoe = temp1*(ubar*tauxx+vbar*tauxy+wbar*tauxz) + temp0*(ubar&
&           *tauxy+vbar*tauyy+wbar*tauyz) + temp*(ubar*tauxz+vbar*tauyz+&
&           wbar*tauzz) - temp2*q_x - temp3*q_y - temp4*q_z
! update the residuals of cell i and i+1.
          fwd(i, j, k, imx) = fwd(i, j, k, imx) - fmxd
          fw(i, j, k, imx) = fw(i, j, k, imx) - fmx
          fwd(i, j, k, imy) = fwd(i, j, k, imy) - fmyd
...
! and determine one/ length of vector squared
          temp4 = one/(ssx*ssx+ssy*ssy+ssz*ssz)
          ssd = -(temp4*(2*ssx*ssxd+2*ssy*ssyd+2*ssz*sszd)/(ssx**2+ssy**&
&           2+ssz**2))
          ss = temp4
          ssxd = ssx*ssd + ss*ssxd
          ssx = ss*ssx
...
! compute the viscous fluxes for this j-face.
          ...
          temp3 = sj(i, j, k, 2)
          temp2 = sj(i, j, k, 3)
          fmxd = temp4*tauxxd + temp3*tauxyd + temp2*tauxzd
          fmx = temp4*tauxx + temp3*tauxy + temp2*tauxz
          temp4 = sj(i, j, k, 1)
          temp3 = sj(i, j, k, 2)
          temp2 = sj(i, j, k, 3)
          fmyd = temp4*tauxyd + temp3*tauyyd + temp2*tauyzd
          fmy = temp4*tauxy + temp3*tauyy + temp2*tauyz
          temp4 = sj(i, j, k, 1)
          temp3 = sj(i, j, k, 2)
          temp2 = sj(i, j, k, 3)
          fmzd = temp4*tauxzd + temp3*tauyzd + temp2*tauzzd
          fmz = temp4*tauxz + temp3*tauyz + temp2*tauzz
          temp4 = sj(i, j, k, 1)
          temp3 = sj(i, j, k, 2)
          temp2 = sj(i, j, k, 3)
          temp1 = sj(i, j, k, 1)
          temp0 = sj(i, j, k, 2)
          temp = sj(i, j, k, 3)
          frhoed = temp4*(tauxx*ubard+ubar*tauxxd+tauxy*vbard+vbar*&
&           tauxyd+tauxz*wbard+wbar*tauxzd) + temp3*(tauxy*ubard+ubar*&
&           tauxyd+tauyy*vbard+vbar*tauyyd+tauyz*wbard+wbar*tauyzd) + &
&           temp2*(tauxz*ubard+ubar*tauxzd+tauyz*vbard+vbar*tauyzd+tauzz&
&           *wbard+wbar*tauzzd) - temp1*q_xd - temp0*q_yd - temp*q_zd
          frhoe = temp4*(ubar*tauxx+vbar*tauxy+wbar*tauxz) + temp3*(ubar&
&           *tauxy+vbar*tauyy+wbar*tauyz) + temp2*(ubar*tauxz+vbar*tauyz&
&           +wbar*tauzz) - temp1*q_x - temp0*q_y - temp*q_z
! update the residuals of cell j and j+1.
          fwd(i, j, k, imx) = fwd(i, j, k, imx) - fmxd
          fw(i, j, k, imx) = fw(i, j, k, imx) - fmx
          fwd(i, j, k, imy) = fwd(i, j, k, imy) - fmyd
...
! a 5-cell stencil for doing the pc
            dis2d = (fis2*min1+sigma*fis4)*rradd
...
! modification for fd preconditioner
            dis2d = (fis2*min2+sigma*fis4)*rradd
...
! modification for fd preconditioner
            dis2d = (fis2*min3+sigma*fis4)*rradd
...
!
    ...
    use inputphysicsiff
    use iteration
    use utils_diff, only : getcorrectfork
    implicit none
!
!      local parameters.
!
...
&             drv*vavgd-vavg*drvd+dred-drw*wavgd-wavg*drwd) - gm53*drkd
            abv4 = gm1*(alphaavg*dr-uavg*dru-vavg*drv-wavg*drw+dre) - &
&             gm53*drk
            abv5d = sx*drud + sy*drvd + sz*drwd - dr*unavgd - unavg*drd
            abv5 = sx*dru + sy*drv + sz*drw - unavg*dr
            abv6d = ova2avg*(abv4*abv3d+abv3*abv4d) + abv3*abv4*ova2avgd&
&             + ovaavg*(abv5*abv2d+abv2*abv5d) + abv2*abv5*ovaavgd
            abv6 = abv3*abv4*ova2avg + abv2*abv5*ovaavg
...
! density.
            ...
            fw(i, j, k, irho) = fw(i, j, k, irho) - fs
! x-momentum.
            fsd = dru*lam3d + lam3*drud + abv6*uavgd + uavg*abv6d + sx*&
&             abv7d
            fs = lam3*dru + uavg*abv6 + sx*abv7
            fwd(i+1, j, k, imx) = fwd(i+1, j, k, imx) + fsd
            fw(i+1, j, k, imx) = fw(i+1, j, k, imx) + fs
            ...
            fw(i, j, k, imx) = fw(i, j, k, imx) - fs
! y-momentum.
            fsd = drv*lam3d + lam3*drvd + abv6*vavgd + vavg*abv6d + sy*&
&             abv7d
            fs = lam3*drv + vavg*abv6 + sy*abv7
            fwd(i+1, j, k, imy) = fwd(i+1, j, k, imy) + fsd
            fw(i+1, j, k, imy) = fw(i+1, j, k, imy) + fs
            ...
            fw(i, j, k, imy) = fw(i, j, k, imy) - fs
! z-momentum.
            fsd = drw*lam3d + lam3*drwd + abv6*wavgd + wavg*abv6d + sz*&
&             abv7d
            fs = lam3*drw + wavg*abv6 + sz*abv7
            fwd(i+1, j, k, imz) = fwd(i+1, j, k, imz) + fsd
            fw(i+1, j, k, imz) = fw(i+1, j, k, imz) + fs
