...
!
      ...
      temp = (ya-yc)*(zb-zd) - (za-zc)*(yb-yd)
      temp0 = xp - fourth*(xa+xb+xc+xd)
      temp1 = (za-zc)*(xb-xd) - (xa-xc)*(zb-zd)
      temp2 = yp - fourth*(ya+yb+yc+yd)
      temp3 = (xa-xc)*(yb-yd) - (ya-yc)*(xb-xd)
      temp4 = zp - fourth*(za+zb+zc+zd)
      volumed = temp*(xpd-fourth*(xad+xbd+xcd+xdd)) + temp0*((zb-zd)*(&
&       yad-ycd)+(ya-yc)*(zbd-zdd)-(yb-yd)*(zad-zcd)-(za-zc)*(ybd-ydd)) &
&       + temp1*(ypd-fourth*(yad+ybd+ycd+ydd)) + temp2*((xb-xd)*(zad-zcd&
&       )+(za-zc)*(xbd-xdd)-(zb-zd)*(xad-xcd)-(xa-xc)*(zbd-zdd)) + temp3&
&       *(zpd-fourth*(zad+zbd+zcd+zdd)) + temp4*((yb-yd)*(xad-xcd)+(xa-&
&       xc)*(ybd-ydd)-(xb-xd)*(yad-ycd)-(ya-yc)*(xbd-xdd))
      volume = temp0*temp + temp2*temp1 + temp4*temp3
...
! either -0.5 or 0.5.
      sid(i, j, k, 1) = fact*(v2(3)*v1d(2)+v1(2)*v2d(3)-v2(2)*v1d(3)-v1(&
&       3)*v2d(2))
      si(i, j, k, 1) = fact*(v1(2)*v2(3)-v1(3)*v2(2))
      sid(i, j, k, 2) = fact*(v2(1)*v1d(3)+v1(3)*v2d(1)-v2(3)*v1d(1)-v1(&
&       1)*v2d(3))
      si(i, j, k, 2) = fact*(v1(3)*v2(1)-v1(1)*v2(3))
      sid(i, j, k, 3) = fact*(v2(2)*v1d(1)+v1(1)*v2d(2)-v2(1)*v1d(2)-v1(&
&       2)*v2d(1))
...
! diagonal vectors times fact; remember that fact is
! either -0.5 or 0.5.
      sjd(i, j, k, 1) = fact*(v2(3)*v1d(2)+v1(2)*v2d(3)-v2(2)*v1d(3)-v1(&
&       3)*v2d(2))
      sj(i, j, k, 1) = fact*(v1(2)*v2(3)-v1(3)*v2(2))
      sjd(i, j, k, 2) = fact*(v2(1)*v1d(3)+v1(3)*v2d(1)-v2(3)*v1d(1)-v1(&
&       1)*v2d(3))
      sj(i, j, k, 2) = fact*(v1(3)*v2(1)-v1(1)*v2(3))
      sjd(i, j, k, 3) = fact*(v2(2)*v1d(1)+v1(1)*v2d(2)-v2(1)*v1d(2)-v1(&
&       2)*v2d(1))
...
! diagonal vectors times fact; remember that fact is
! either -0.5 or 0.5.
      skd(i, j, k, 1) = fact*(v2(3)*v1d(2)+v1(2)*v2d(3)-v2(2)*v1d(3)-v1(&
&       3)*v2d(2))
      sk(i, j, k, 1) = fact*(v1(2)*v2(3)-v1(3)*v2(2))
      skd(i, j, k, 2) = fact*(v2(1)*v1d(3)+v1(3)*v2d(1)-v2(3)*v1d(1)-v1(&
&       1)*v2d(3))
      sk(i, j, k, 2) = fact*(v1(3)*v2(1)-v1(1)*v2(3))
      skd(i, j, k, 3) = fact*(v2(2)*v1d(1)+v1(1)*v2d(2)-v2(1)*v1d(2)-v1(&
&       2)*v2d(1))
...
! and possibly correct for inward pointing.
        arg1d = 2*xxp*xxpd + 2*yyp*yypd + 2*zzp*zzpd
        arg1 = xxp*xxp + yyp*yyp + zzp*zzp
        temp = sqrt(arg1)
        if (arg1 .eq. 0.0_8) then
          factd = 0.0_8
        else
          factd = arg1d/(2.0*temp)
        end if
        fact = temp
...
! compute the unit normal.
        bcdatad(mm)%norm(i, j, 1) = xxp*factd + fact*xxpd
        bcdata(mm)%norm(i, j, 1) = fact*xxp
        bcdatad(mm)%norm(i, j, 2) = yyp*factd + fact*yypd
        bcdata(mm)%norm(i, j, 2) = fact*yyp
        bcdatad(mm)%norm(i, j, 3) = zzp*factd + fact*zzpd
...
!   plus diff mem management of: dw:in
  subroutine resscale_d()
    use constants
    use blockpointersiff, only : il, jl, kl, nx, ny, nz, volref, dw, &
&   dwd
    use flowvarrefstateiff, only : nwf, nt1, nt2
...
!   plus diff mem management of: dw:in fw:in
  subroutine sumdwandfw_d()
    use constants
    use blockpointersiff, only : il, jl, kl, dw, dwd, fw, fwd, iblank
    use flowvarrefstateiff, only : nwf
    implicit none
! local variables
  ...
  subroutine sumdwandfw()
    use constants
    use blockpointersiff, only : il, jl, kl, dw, fw, iblank
    use flowvarrefstateiff, only : nwf
    implicit none
! local variables
    integer(kind=inttype) :: i, j, k, l
