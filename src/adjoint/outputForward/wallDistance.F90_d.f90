...
!
module walldistance_diff
  use constants, only : inttype, realtype
  use walldistancedataiff
  implicit none
! ----------------------------------------------------------------------
!                                                                      |
...
! pointers are already set.
    use constants
    use blockpointersiff, only : nx, ny, nz, il, jl, kl, x, xd, &
&   flowdoms, d2wall, d2walld
    implicit none
...
! functions o to get target: (ccw ordering remember!)
            temp = (one-u)*(one-v)
            xpd(:) = temp*xsurfd(3*(ind(1)-1)+1:3*ind(1)) + u*(one-v)*&
&             xsurfd(3*(ind(2)-1)+1:3*ind(2)) + u*v*xsurfd(3*(ind(3)-1)+&
&             1:3*ind(3)) + (one-u)*v*xsurfd(3*(ind(4)-1)+1:3*ind(4))
            xp(:) = temp*xsurf(3*(ind(1)-1)+1:3*ind(1)) + u*(one-v)*&
&             xsurf(3*(ind(2)-1)+1:3*ind(2)) + u*v*xsurf(3*(ind(3)-1)+1:&
&             3*ind(3)) + (one-u)*v*xsurf(3*(ind(4)-1)+1:3*ind(4))
! get the cell center
...
! pointers are already set.
    use constants
    use blockpointersiff, only : nx, ny, nz, il, jl, kl, x, flowdoms, &
&   d2wall
    implicit none
! subroutine arguments
    integer(kind=inttype) :: nn, level, sps
