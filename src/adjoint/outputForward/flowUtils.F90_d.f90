...
!
    use constants
    use inputphysicsiff, only : cpmodel
    use flowvarrefstateiff, only : rgas, rgasd, gammainf
...
!
    use constants
    use inputphysicsiff, only : cpmodel
    use flowvarrefstateiff, only : rgas, gammainf
    use utils_diff, only : terminate
    implicit none
!
!      subroutine arguments.
...
!
    use constants
    use cpcurvefits
    use flowvarrefstateiff, only : tref, trefd, rgas, rgasd, gammainf
    use inputphysicsiff, only : cpmodel
    implicit none
    real(kind=realtype), intent(in) :: rho, p, u, v, w
    real(kind=realtype), intent(in) :: rhod, pd, ud, vd, wd
...
! formula is valid.
      ...
      kin = half*(u*u+v*v+w*w)
      temp = rho*kin/(govgm1*p)
      pwx1d = (kin*rhod+rho*kind0-temp*govgm1*pd)/(govgm1*p)
      pwx1 = one + temp
      if (pwx1 .le. 0.0_8 .and. (govgm1 .eq. 0.0_8 .or. govgm1 .ne. int(&
&         govgm1))) then
        pwr1d = 0.0_8
      else
...
!
    use constants
    use cpcurvefits
    use flowvarrefstateiff, only : tref, rgas, gammainf
    use inputphysicsiff, only : cpmodel
    implicit none
    real(kind=realtype), intent(in) :: rho, p, u, v, w
    real(kind=realtype), intent(out) :: ptot
...
!
    use constants
    use blockpointersiff, only : ie, je, ke, w, wd, p, pd, aa, aad, &
&   gamma
...
!
    use constants
    use blockpointersiff, only : ie, je, ke, w, p, aa, gamma
    use utils_diff, only : getcorrectfork
    implicit none
!
!      local variables.
...
!
    use constants
    use blockpointersiff, only : w, wd, p, pd
    use flowvarrefstateiff, only : rgas, rgasd, tref, trefd
    use inputphysicsiff, only : cpmodel, gammaconstant
    implicit none
!
!      subroutine arguments.
...
! step of the energy.
      ...
      if (correctfork) then
        do k=kstart,kend
          do j=jstart,jend
            do i=istart,iend
              temp = w(i, j, k, ivx)*w(i, j, k, ivx) + w(i, j, k, ivy)*w&
&               (i, j, k, ivy) + w(i, j, k, ivz)*w(i, j, k, ivz)
              temp0 = w(i, j, k, irho)
              temp1 = w(i, j, k, itu1)
              temp2 = w(i, j, k, irho)
              wd(i, j, k, irhoe) = ovgm1*pd(i, j, k) + half*(temp*wd(i, &
&               j, k, irho)+temp0*(2*w(i, j, k, ivx)*wd(i, j, k, ivx)+2*&
&               w(i, j, k, ivy)*wd(i, j, k, ivy)+2*w(i, j, k, ivz)*wd(i&
&               , j, k, ivz))) - factk*(temp1*wd(i, j, k, irho)+temp2*wd&
&               (i, j, k, itu1))
              w(i, j, k, irhoe) = ovgm1*p(i, j, k) + half*(temp0*temp) -&
&               factk*(temp2*temp1)
            end do
          end do
        end do
        ...
        do k=kstart,kend
          do j=jstart,jend
            do i=istart,iend
              temp2 = w(i, j, k, ivx)*w(i, j, k, ivx) + w(i, j, k, ivy)*&
&               w(i, j, k, ivy) + w(i, j, k, ivz)*w(i, j, k, ivz)
              temp1 = w(i, j, k, irho)
              wd(i, j, k, irhoe) = ovgm1*pd(i, j, k) + half*(temp2*wd(i&
&               , j, k, irho)+temp1*(2*w(i, j, k, ivx)*wd(i, j, k, ivx)+&
&               2*w(i, j, k, ivy)*wd(i, j, k, ivy)+2*w(i, j, k, ivz)*wd(&
&               i, j, k, ivz)))
...
&           irho)+temp*v2d))
          p(i, j, k) = gm1*(w(i, j, k, irhoe)-half*(temp*v2))
...
!
    ...
    use inputphysicsiff, only : cpmodel, gammaconstant
    use blockpointersiff, only : w, p
    use flowvarrefstateiff, only : kpresent, pinf, rgas, tref
    use cpcurvefits
    implicit none
!
!      subroutine arguments.
...
&               temp
              t = pp/temp
              temp1 = (t/tsuth)**1.5_realtype
              temp0 = musuth*(tsuth+ssuth)/(t+ssuth)
              rlvd(i, j, k) = temp1*((tsuth+ssuth)*musuthd+musuth*(&
&               tsuthd+ssuthd)-temp0*(td+ssuthd))/(t+ssuth) + temp0*&
&               1.5_realtype*(t/tsuth)**0.5*(td-t*tsuthd/tsuth)/tsuth
              rlv(i, j, k) = temp0*temp1
            end do
          end do
        end do
      else
...
! nondimensional laminar viscosity.
              temp0 = w(i, j, k, irho)
              temp = p(i, j, k)/(rgas*temp0)
              td = (pd(i, j, k)-temp*(temp0*rgasd+rgas*wd(i, j, k, irho)&
&               ))/(rgas*temp0)
              t = temp
              temp1 = (t/tsuth)**1.5_realtype
              temp0 = musuth*(tsuth+ssuth)/(t+ssuth)
...
!                beta:in dragdirection:out liftdirection:out
  subroutine adjustinflowangle_d()
    use constants
    use inputphysicsiff, only : alpha, alphad, beta, betad, liftindex,&
...
&                 liftdirection, liftdirectiond, liftindex)
  ...
  subroutine adjustinflowangle()
    use constants
    use inputphysicsiff, only : alpha, beta, liftindex, &
&   veldirfreestream, liftdirection, dragdirection
    implicit none
!local vars
    real(kind=realtype), dimension(3) :: refdir1, refdir2
! velocity direction given by the rotation of a unit vector
...
!
    ...
    use inputmotion
    use monitor
    use utils_diff, only : rigidrotangle, derivativerigidrotangle
    implicit none
!
!      subroutine arguments.
!
...
!                *w:in *qx:out *qy:out *qz:out *vol:in *ux:out
