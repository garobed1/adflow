...
!   with respect to varying inputs: saconsts timeref *w *rlv *vol
!                *d2wall *si *sj *sk
!   rw status of diff variables: saconsts:in timeref:in *w:in *rlv:in
!                *scratch:out *vol:in *d2wall:in *si:in *sj:in
!                *sk:in rsak:(loc) rsacv1:(loc) rsacw1:(loc) rsacw2:(loc)
!                rsacw3:(loc) rsact3:(loc) rsact4:(loc) rsacrot:(loc)
!                rsacb1:(loc) rsacb2:(loc) rsacb3:(loc) cw36:(loc)
!                cv13:(loc) kar2inv:(loc)
...
&   syzd
    ...
    call saassign_d()
! set model constants
    cv13d = 3*rsacv1**2*rsacv1d
    cv13 = rsacv1**3
    temp = one/(rsak*rsak)
    kar2invd = -(temp*2*rsakd/rsak)
    kar2inv = temp
    cw36d = 6*rsacw3**5*rsacw3d
    cw36 = rsacw3**6
    cb3inv = one/rsacb3
...
! the production term near a viscous wall.
            ...
            chi3 = chi*chi2
            temp10 = chi3/(chi3+cv13)
            fv1d = (chi3d-temp10*(chi3d+cv13d))/(chi3+cv13)
            fv1 = temp10
...
! this function should be set to 0.0.
            if (useft2sa) then
              temp10 = exp(-(rsact4*chi2))
              ft2d = temp10*rsact3d - rsact3*exp(-(rsact4*chi2))*(chi2*&
&               rsact4d+rsact4*chi2d)
              ft2 = rsact3*temp10
            else
              ft2 = zero
              ft2d = 0.0_8
...
! of the wall.
            temp10 = fv2*kar2inv*dist2inv
            temp9 = w(i, j, k, itu1)
            sstd = ssd + temp10*wd(i, j, k, itu1) + temp9*(dist2inv*(&
&             kar2inv*fv2d+fv2*kar2invd)+fv2*kar2inv*dist2invd)
            sst = ss + temp9*temp10
! add rotation term (userotationsa defined in inputparams.f90)
            if (userotationsa) then
              ...
              sstd = sstd + min1*rsacrotd + rsacrot*min1d
...
! asymptotical value of fw is then already reached.
            temp10 = kar2inv*dist2inv/sst
            temp9 = w(i, j, k, itu1)
            rrd = temp10*wd(i, j, k, itu1) + temp9*(dist2inv*kar2invd+&
&             kar2inv*dist2invd-temp10*sstd)/sst
            rr = temp9*temp10
            ...
            temp10 = rr**6 - rr
            ggd = (rsacw2*(6*rr**5-1.0)+1.0)*rrd + temp10*rsacw2d
            gg = rr + rsacw2*temp10
            gg6d = 6*gg**5*ggd
            gg6 = gg**6
            temp10 = (one+cw36)/(gg6+cw36)
            pwx1d = (cw36d-temp10*(gg6d+cw36d))/(gg6+cw36)
            pwx1 = temp10
            if (pwx1 .le. 0.0_8 .and. (sixth .eq. 0.0_8 .or. sixth .ne. &
&               int(sixth))) then
              termfwd = 0.0_8
            else
...
! linearization. the source term is stored in dvt.
            if (approxsa) then
              ...
              term1d = (one-ft2)*(ss*rsacb1d+rsacb1*ssd) - rsacb1*ss*&
&               ft2d
              term1 = rsacb1*(one-ft2)*ss
            end if
            temp10 = (one-ft2)*fv2 + ft2
            temp9 = kar2inv*rsacb1*temp10 - rsacw1*fwsa
            term2d = temp9*dist2invd + dist2inv*(temp10*(rsacb1*kar2invd&
&             +kar2inv*rsacb1d)+kar2inv*rsacb1*((one-ft2)*fv2d-(fv2-1.0)&
&             *ft2d)-fwsa*rsacw1d-rsacw1*fwsad)
...
!                *si *sj *sk
!   rw status of diff variables: saconsts:in *w:in *rlv:in *scratch:in-out
!                *vol:in *si:in *sj:in *sk:in rsak:(loc) rsacv1:(loc)
!                rsacw1:(loc) rsacw2:(loc) rsacw3:(loc) rsact3:(loc)
!                rsact4:(loc) rsacrot:(loc) rsacb1:(loc) rsacb2:(loc)
!                rsacb3:(loc) cb3inv:(loc)
!   plus diff mem management of: w:in rlv:in scratch:in vol:in
!                si:in sj:in sk:in
...
! these coefficients are nonnegative.
          temp = w(i, j, k, itu1)
          cnudd = -(rsacb2*cb3inv*wd(i, j, k, itu1)+temp*(cb3inv*rsacb2d&
&           +rsacb2*cb3invd))
          cnud = -(temp*(rsacb2*cb3inv))
...
&           nud)
          ...
          cdmd = ttm*cb3inv*(numd+nutm*rsacb2d+(one+rsacb2)*nutmd) + &
&           temp0*(cb3inv*ttmd+ttm*cb3invd)
          cdm = temp0*(ttm*cb3inv)
          temp0 = nup + (one+rsacb2)*nutp
          cdpd = ttp*cb3inv*(nupd+nutp*rsacb2d+(one+rsacb2)*nutpd) + &
&           temp0*(cb3inv*ttpd+ttp*cb3invd)
          cdp = temp0*(ttp*cb3inv)
...
! these coefficients are nonnegative.
          temp1 = w(i, j, k, itu1)
          cnudd = -(rsacb2*cb3inv*wd(i, j, k, itu1)+temp1*(cb3inv*&
&           rsacb2d+rsacb2*cb3invd))
          cnud = -(temp1*(rsacb2*cb3inv))
          camd = cnud*ttmd + ttm*cnudd
          cam = ttm*cnud
          capd = cnud*ttpd + ttp*cnudd
          cap = ttp*cnud
...
&           +nud)
          ...
          cdmd = ttm*cb3inv*(numd+nutm*rsacb2d+(one+rsacb2)*nutmd) + &
&           temp1*(cb3inv*ttmd+ttm*cb3invd)
          cdm = temp1*(ttm*cb3inv)
          temp1 = nup + (one+rsacb2)*nutp
          cdpd = ttp*cb3inv*(nupd+nutp*rsacb2d+(one+rsacb2)*nutpd) + &
&           temp1*(cb3inv*ttpd+ttp*cb3invd)
          cdp = temp1*(ttp*cb3inv)
          if (cdm + cam .lt. zero) then
            c1m = zero
            c1md = 0.0_8
          else
