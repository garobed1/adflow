...
&   funcvaluesd)
    ...
    use flowvarrefstateiff, only : pref, prefd, rhoref, rhorefd, tref,&
&   trefd, lref, gammainf, pinf, pinfd, uref, urefd, uinf, uinfd
    use inputphysicsiff, only : liftdirection, liftdirectiond, &
...
&     'completely verifed from scratch'
  ...
  subroutine getcostfunctions(globalvals, funcvalues)
    use constants
    use inputtimespectral, only : ntimeintervalsspectral
    use flowvarrefstateiff, only : pref, rhoref, tref, lref, gammainf,&
&   pinf, uref, uinf
    use inputphysicsiff, only : liftdirection, dragdirection, &
...
!
    ...
    use inputphysicsiff, only : machcoef, machcoefd, pointref, &
...
&       j+1, 3))
      zc = fourth*(xx(i, j, 3)+xx(i+1, j, 3)+xx(i, j+1, 3)+xx(i+1, j+1, &
&       3))
      sepsensoravgd(1) = sepsensoravgd(1) + xc*sensord + sensor*xcd
      sepsensoravg(1) = sepsensoravg(1) + sensor*xc
      sepsensoravgd(2) = sepsensoravgd(2) + yc*sensord + sensor*ycd
      sepsensoravg(2) = sepsensoravg(2) + sensor*yc
      sepsensoravgd(3) = sepsensoravgd(3) + zc*sensord + sensor*zcd
      ...
      if (computecavitation) then
        ...
        temp = -(10*2*sensor1)
        temp0 = one/(one+exp(temp))
        sensor1d = temp0*exp(temp)*10*2*sensor1d/(one+exp(temp))
        sensor1 = temp0
...
!
    ...
    use flowvarrefstateiff
    use inputcostfunctions
    use inputphysicsiff, only : machcoef, pointref, veldirfreestream, &
...
!                pref:in rgas:in rhoref:in *xx:in *pp1:in *pp2:in
...
&   mm)
    use constants
    use blockpointersiff, only : bctype, bcfaceid, bcdata, bcdatad, &
&   addgridvelocities
...
! also, the sign is flipped between inflow and outflow types
      temp = fact*blk*internalflowfact*inflowfact
      temp0 = massflowratelocal/(timeref*cellarea)
      massflowratelocald = temp*(massflowratelocald-temp0*(cellarea*&
&       timerefd+timeref*cellaread))/(timeref*cellarea)
      massflowratelocal = temp*temp0
...
&     mmom
  ...
  subroutine flowintegrationface(isinflow, localvalues, mm)
    use constants
    use blockpointersiff, only : bctype, bcfaceid, bcdata, &
&   addgridvelocities
    use flowvarrefstateiff, only : pref, pinf, rhoref, timeref, lref, &
