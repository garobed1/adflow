...
! apply bc's for a single block
    use constants
    use blockpointers_diff, only : nbocos, bctype, nviscbocos, w, dw, x,&
...
!  planes, i.e. a 2d problem.
    use constants
    use blockpointers_diff, only : bcdata, bcdatad
    use flowvarrefstate_diff, only : viscous, eddymodel
...
!  planes, i.e. a 2d problem.
    use constants
    use blockpointers_diff, only : bcdata
    use flowvarrefstate_diff, only : viscous, eddymodel
...
! eddy viscosity in the halo.
      ...
      ww2d(i, j, irho) = ww2d(i, j, irho) + ww1d(i, j, irho)
      ww1d(i, j, irho) = 0.0_8
      tmpd = nnz*vtzd + nny*vtyd + nnx*vtxd
      tempd0 = two*tmpd
      nnzd = tmp*vtzd + ww2(i, j, ivz)*tempd0
      nnyd = tmp*vtyd + ww2(i, j, ivy)*tempd0
      nnxd = tmp*vtxd + ww2(i, j, ivx)*tempd0
      call popreal8(tmp)
      ww2d(i, j, ivx) = ww2d(i, j, ivx) + nnx*tempd0
      ww2d(i, j, ivy) = ww2d(i, j, ivy) + nny*tempd0
      ww2d(i, j, ivz) = ww2d(i, j, ivz) + nnz*tempd0
      call popreal8(nnz)
      call popreal8(nny)
      call popreal8(nnx)
      tmpd = nnz*nnzd + nny*nnyd + nnx*nnxd
      temp = nnx*nnx + nny*nny + nnz*nnz
      temp0 = sqrt(temp)
      if (temp .eq. 0.0_8) then
        tempd = 0.0_8
      else
        tempd = -(one*tmpd/(2.0*temp0**3))
      end if
      nnzd = tmp*nnzd + 2*nnz*tempd
      nnyd = tmp*nnyd + 2*nny*tempd
      nnxd = tmp*nnxd + 2*nnx*tempd
...
! eddy viscosity in the halo.
      ...
      tmpd = nnz*vtzd + nny*vtyd + nnx*vtxd
      tempd0 = two*tmpd
      nnzd = tmp*vtzd + ww3(i, j, ivz)*tempd0
      nnyd = tmp*vtyd + ww3(i, j, ivy)*tempd0
      nnxd = tmp*vtxd + ww3(i, j, ivx)*tempd0
      call popreal8(tmp)
      ww3d(i, j, ivx) = ww3d(i, j, ivx) + nnx*tempd0
      ww3d(i, j, ivy) = ww3d(i, j, ivy) + nny*tempd0
      ww3d(i, j, ivz) = ww3d(i, j, ivz) + nnz*tempd0
      call popreal8(nnz)
      call popreal8(nny)
      call popreal8(nnx)
      tmpd = nnz*nnzd + nny*nnyd + nnx*nnxd
...
&   pp3, pp3d, rev0, rev0d, rev1, rev1d, rev2, rev2d, istart, jstart, &
&   isize, jsize
    use flowvarrefstate_diff, only : viscous, eddymodel, rgas, rgasd
    use iteration, only : currentlevel, groundlevel
    implicit none
! subroutine arguments.
    logical, intent(in) :: secondhalo, correctfork
...
! the wall.
      ...
      tempd0 = -(pp1(i, j)*tempd/(rgas*t1))
      rgasd = rgasd + t1*tempd0
      t1d = rgas*tempd0
...
! already set
    ...
    use bcpointers_diff, only : ww0, ww1, ww2, rlv0, rlv1, rlv2, pp0, &
&   pp1, pp2, pp3, rev0, rev1, rev2, istart, jstart, isize, jsize
    use flowvarrefstate_diff, only : viscous, eddymodel, rgas
    use iteration, only : currentlevel, groundlevel
    implicit none
! subroutine arguments.
    logical, intent(in) :: secondhalo, correctfork
...
&   jstart
    use flowvarrefstate_diff, only : eddymodel, viscous
    implicit none
! subroutine arguments.
    logical, intent(in) :: secondhalo, correctfork
    integer(kind=inttype), intent(in) :: nn
...
! in the donor cell.
      ...
      call popcontrol1b(branch)
      if (branch .eq. 0) then
        ww1d(i, j, irho) = ww1d(i, j, irho) + ww1(i, j, itu1)*twothird*&
&         pp1d(i, j)
        ww1d(i, j, itu1) = ww1d(i, j, itu1) + ww1(i, j, irho)*twothird*&
&         pp1d(i, j)
      end if
      ...
      ued = ww1d(i, j, ivx) + nnx*qned
      nnxd = (qnh-qne)*ww1d(i, j, ivx) + ue*qned
      ww1d(i, j, ivx) = 0.0_8
      ad = -(two*ovgm1*qnhd)
      temp = pexit/ww1(i, j, irho)
      if (gamma2(i, j)*temp .eq. 0.0_8) then
        tempd = 0.0_8
      else
        tempd = gamma2(i, j)*ad/(ww1(i, j, irho)*2.0*sqrt(gamma2(i, j)*&
&         temp))
      end if
      pexitd = tempd
      ww1d(i, j, irho) = ww1d(i, j, irho) - temp*tempd
      if (pexit/ss .le. 0.0_8 .and. (ovg .eq. 0.0_8 .or. ovg .ne. int(&
&         ovg))) then
        tempd = 0.0_8
      else
        tempd = ovg*(pexit/ss)**(ovg-1)*ww1d(i, j, irho)/ss
      end if
      pexitd = pexitd + pp1d(i, j) + tempd
      pp1d(i, j) = 0.0_8
      ww1d(i, j, irho) = 0.0_8
      ssd = -(pexit*tempd/ss)
      ad = two*ovgm1*acd
      ...
      pintd = r**temp*ssd + r*tempd
      if (r .le. 0.0_8 .and. (temp .eq. 0.0_8 .or. temp .ne. int(temp))&
&     ) then
        rd = pint*tempd
      else
        rd = temp*r**(temp-1)*pint*ssd + pint*tempd
      end if
      ww2d(i, j, irho) = ww2d(i, j, irho) - one*rd/ww2(i, j, irho)**2
      call popcontrol1b(branch)
      if (branch .eq. 0) then
        ww2d(i, j, irho) = ww2d(i, j, irho) - ww2(i, j, itu1)*twothird*&
&         pintd
        ww2d(i, j, itu1) = ww2d(i, j, itu1) - ww2(i, j, irho)*twothird*&
&         pintd
      end if
...
&   rev0, rev0d, rev1, rev1d, rev2, rev2d, gamma2, isize, jsize, istart,&
&   jstart
    use inputphysics_diff, only : cpmodel, gammaconstant
...
! in the donor cell.
          ...
          if (branch .lt. 2) then
            if (branch .eq. 0) then
              ...
              ww1d(i, j, irho) = ww1d(i, j, irho) + ww1(i, j, itu1)*&
&               twothird*pp1d(i, j)
              ww1d(i, j, itu1) = ww1d(i, j, itu1) + ww1(i, j, irho)*&
&               twothird*pp1d(i, j)
            end if
          ...
          tempd1 = ww1d(i, j, irho)/(rgas*ts)
          ww1d(i, j, irho) = 0.0_8
          ptotd = ratio*tempd1 + ratio*pp1d(i, j)
          ratiod = ptot*tempd1 + ptot*pp1d(i, j)
          tsd = -(ptot*ratio*tempd1/ts)
          pp1d(i, j) = 0.0_8
          if (ts/ttot .le. 0.0_8 .and. (govgm1 .eq. 0.0_8 .or. govgm1 &
&             .ne. int(govgm1))) then
            tempd1 = 0.0_8
          else
            tempd1 = govgm1*(ts/ttot)**(govgm1-1)*ratiod/ttot
          end if
          tsd = tsd + tempd1
          ttotd = -(ts*tempd1/ttot)
 ...
 100      qd = ssz*ww1d(i, j, ivz)
          ...
          bbd = bbd + 2*bb*ddd
          aa2d = aa2d - cc*four*ddd
          ccd = -(aa2*four*ddd)
          betad = 2*beta*half*gm1*ccd - alpha*gm1*bbd
          a2totd = a2totd - two*ovgm1*ccd
          alphad = 2*alpha*half*gm1*aa2d - beta*gm1*bbd
          nnxd = ssx*alphad
          nnyd = ssy*alphad
          nnzd = ssz*alphad
          a2 = gamma2(i, j)*pp2(i, j)*r
          ...
          if (branch .ne. 0) then
            ...
            if (temp0 .eq. 0.0_8) then
              ...
              tempd0 = scalefactd/(r*temp*2.0*sqrt(temp0))
            end if
            htotd = htotd + tempd0
            tempd1 = -(temp0*tempd0)
            rd = rd + temp*tempd1
...
! in the donor cell.
          ...
          tempd1 = half*gm1*a2d
          betad = tempd1
          nnxd = ww2(i, j, ivx)*betad - velx*tempd1
          nnyd = ww2(i, j, ivy)*betad - vely*tempd1
          nnzd = ww2(i, j, ivz)*betad - velz*tempd1
          a2 = gamma2(i, j)*pp2(i, j)*r
          ww2d(i, j, ivx) = ww2d(i, j, ivx) + nnx*betad
          ww2d(i, j, ivy) = ww2d(i, j, ivy) + nny*betad
          ww2d(i, j, ivz) = ww2d(i, j, ivz) + nnz*betad
...
!  correct block on the correct grid level.
    use constants
    use blockpointers_diff, only : bcdata, bcdatad, addgridvelocities
    use flowvarrefstate_diff, only : viscous, eddymodel, rgas, rgasd
    use inputdiscretization, only : eulerwallbctreatment
    use iteration, only : currentlevel, groundlevel
    use utils_diff, only : mydim, mydim_b
    use bcpointers_diff, only : ww0, ww0d, ww1, ww1d, ww2, ww2d, pp0, &
...
! be that an additional term is needed there.
          ...
          qkd = temp1*tempd0
          tempd2 = qk*tempd0
          uuxd = rxk*tempd2 + rxj*tempd1 + skxa*qkd + sjxa*qjd
          rxkd = uux*tempd2
          uuyd = ryk*tempd2 + ryj*tempd1 + skya*qkd + sjya*qjd
          rykd = uuy*tempd2
          uuzd = rzk*tempd2 + rzj*tempd1 + skza*qkd + sjza*qjd
          rzkd = uuz*tempd2
          rxjd = uux*tempd1
          ryjd = uuy*tempd1
          rzjd = uuz*tempd1
          ...
          bcdatad(nn)%norm(j, k, 3) = bcdatad(nn)%norm(j, k, 3) + skza*&
&           rkd + sjza*rjd + siza*rid
          bcdatad(nn)%norm(j, k, 2) = bcdatad(nn)%norm(j, k, 2) + skya*&
&           rkd + sjya*rjd + siya*rid
          ww2d(j, k, ivz) = ww2d(j, k, ivz) + uuzd
          ww2d(j, k, ivy) = ww2d(j, k, ivy) + uuyd
          ww2d(j, k, ivx) = ww2d(j, k, ivx) + uuxd
          bcdatad(nn)%norm(j, k, 1) = bcdatad(nn)%norm(j, k, 1) + skxa*&
&           rkd + sjxa*rjd + sixa*rid
...
!  correct block on the correct grid level.
    use constants
    use blockpointers_diff, only : bcdata, addgridvelocities
    use flowvarrefstate_diff, only : viscous, eddymodel, rgas
    use inputdiscretization, only : eulerwallbctreatment
    use iteration, only : currentlevel, groundlevel
...
! it is assumed that the bcpointers are already set *
    use constants
    use blockpointers_diff, only : bcdata, bcdatad
    use flowvarrefstate_diff, only : eddymodel, viscous, gammainf, winf,&
...
&         ovgm1))) then
        ...
        tempd0 = ovgm1*(sf*cc)**(ovgm1-1)*ww1d(i, j, irho)
      end if
      ccd = ww1(i, j, irho)*pp1d(i, j) + sf*tempd0
      pp1d(i, j) = 0.0_8
      ww1d(i, j, irho) = 0.0_8
      sfd = cc*tempd0
...
&           temp*ww2(i, j, irho)**(temp-1)*tempd
        pp2d(i, j) = pp2d(i, j) - ww2(i, j, irho)**temp*tempd/pp2(i, j)
        wed = wfd
        tempd = bcdata(nn)%norm(i, j, 3)*wfd
        bcdatad(nn)%norm(i, j, 3) = bcdatad(nn)%norm(i, j, 3) + (qnf-qne&
&         )*wfd
        qnfd = tempd
        qned = -tempd
        ved = vfd
        tempd = bcdata(nn)%norm(i, j, 2)*vfd
        bcdatad(nn)%norm(i, j, 2) = bcdatad(nn)%norm(i, j, 2) + (qnf-qne&
&         )*vfd
        qnfd = qnfd + tempd
        qned = qned - tempd
        ued = ufd
        tempd = bcdata(nn)%norm(i, j, 1)*ufd
...
&         )*ufd
      ...
      tempd = fourth*gm1*cfd
      ac1d = tempd + half*qnfd
      ac2d = half*qnfd - tempd
      call popcontrol1b(branch)
      if (branch .eq. 0) then
        qned = qned + ac2d
        ced = -(two*ovgm1*ac2d)
...
!
    use constants
    use blockpointers_diff, only : bctype
    use flowvarrefstate_diff, only : viscous, eddymodel
...
!  currently point.
    use constants
    use cpcurvefits
    use flowvarrefstate_diff, only : rgas, tref
    use inputphysics_diff, only : cpmodel, gammaconstant
...
! only implements the constant cpmodel
    use constants
    use inputphysics_diff, only : gammaconstant, cpmodel
    use utils_diff, only : terminate
    use bcpointers_diff, only : isize, jsize, istart, jstart
    implicit none
    real(kind=realtype), dimension(:, :) :: pp
...
!$bwd-of ii-loop 
      do ii=0,isize*jsize-1
        ...
        if (.not.correctfork) then
          ...
          wwd(i, j, irho) = wwd(i, j, irho) + (ww(i, j, ivx)**2+ww(i, j&
&           , ivy)**2+ww(i, j, ivz)**2)*half*tmpd
          tempd = ww(i, j, irho)*half*tmpd
          ...
          wwd(i, j, irho) = wwd(i, j, irho) + ((ww(i, j, ivx)**2+ww(i, j&
&           , ivy)**2+ww(i, j, ivz)**2)*half-ww(i, j, itu1)*factk)*tmpd0
          tempd = ww(i, j, irho)*half*tmpd0
          wwd(i, j, itu1) = wwd(i, j, itu1) - ww(i, j, irho)*factk*tmpd0
          wwd(i, j, ivx) = wwd(i, j, ivx) + 2*ww(i, j, ivx)*tempd
          wwd(i, j, ivy) = wwd(i, j, ivy) + 2*ww(i, j, ivy)*tempd
          wwd(i, j, ivz) = wwd(i, j, ivz) + 2*ww(i, j, ivz)*tempd
        end if
