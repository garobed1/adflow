...
!
    use constants
    use blockpointers_diff, only : ie, je, ke, il, jl, kl, w, wd, p, pd,&
...
&   addgridvelocities, sectionid
    use flowvarrefstate_diff, only : timeref, timerefd, eddymodel, &
&   gammainf, pinfcorr, pinfcorrd, viscous, rhoinf, rhoinfd
...
&               int(adis))) then
            ...
            rid = (one+one/rij+rki)*radid(i, j, k) - rk*tempd0/ri
            radid(i, j, k) = 0.0_8
            if (rj/rk .le. 0.0_8 .and. (adis .eq. 0.0_8 .or. adis .ne. &
&               int(adis))) then
              tempd0 = 0.0_8
            else
              tempd0 = adis*(rj/rk)**(adis-1)*rjkd/rk
            end if
            rjd = (one+one/rjk+rij)*radjd(i, j, k) + tempd0
            radjd(i, j, k) = 0.0_8
            rkd = rkd - rj*tempd0/rk
            if (ri/rj .le. 0.0_8 .and. (adis .eq. 0.0_8 .or. adis .ne. &
&               int(adis))) then
              tempd0 = 0.0_8
            else
              tempd0 = adis*(ri/rj)**(adis-1)*rijd/rj
            end if
            rid = rid + tempd0
            rjd = rjd - ri*tempd0/rj
          ...
          if (cc2*temp0 .eq. 0.0_8) then
            ...
            tempd = half*rkd/(2.0*sqrt(cc2*temp0))
          end if
          cc2d = temp0*tempd
          tempd0 = cc2*tempd
          sxd = 2*sx*tempd0
          ...
          if (cc2*temp0 .eq. 0.0_8) then
            ...
            tempd = half*rjd/(2.0*sqrt(cc2*temp0))
          end if
          cc2d = cc2d + temp0*tempd
          tempd0 = cc2*tempd
          sxd = 2*sx*tempd0
          syd = 2*sy*tempd0
          szd = 2*sz*tempd0
          ...
          if (cc2*temp0 .eq. 0.0_8) then
            ...
            tempd = half*rid/(2.0*sqrt(cc2*temp0))
          end if
          cc2d = cc2d + temp0*tempd
          tempd0 = cc2*tempd
          sxd = 2*sx*tempd0
...
!
    use constants
    use blockpointers_diff, only : ie, je, ke, il, jl, kl, w, p, rlv, &
&   rev, radi, radj, radk, si, sj, sk, sfacei, sfacej, sfacek, dtl, &
&   gamma, vol, addgridvelocities, sectionid
    use flowvarrefstate_diff, only : timeref, eddymodel, gammainf, &
&   pinfcorr, viscous, rhoinf
    use inputdiscretization, only : adis, dirscaling, radiineededcoarse,&
&   radiineededfine, precond
    use inputphysics_diff, only : equationmode
    use iteration, only : groundlevel, currentlevel
...
!
    ...
    use monitor
    use communication
    use flowutils_diff, only : derivativerotmatrixrigid, getdirvector
    use utils_diff, only : setcoeftimeintegrator, tsalpha, tsbeta, &
...
!
    ...
    use communication
    use flowutils_diff, only : derivativerotmatrixrigid, getdirvector
    use utils_diff, only : tsalpha, tsbeta, tsmach, terminate, &
...
!
    use constants
    use blockpointers_diff, only : il, jl, kl, addgridvelocities, nbocos&
