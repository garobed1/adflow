...
!   gradient     of useful results: timeref *p *w *dw *vol *si
!                *sj *sk
!   with respect to varying inputs: timeref *p *w *dw *vol *si
!                *sj *sk
!   rw status of diff variables: timeref:incr *p:incr *w:incr *dw:in-out
...
!
    use constants
    use blockpointers_diff, only : nx, il, ie, ny, jl, je, nz, kl, ke, &
...
&   factfamilyi, factfamilyj, factfamilyk
    ...
    use flowvarrefstate_diff, only : timeref, timerefd
    use inputphysics_diff, only : equationmode
    implicit none
!
!      local variables.
!
...
&   factfamilyk
    use cgnsgrid, only : cgnsdoms, massflowfamilyinv
    use flowvarrefstate_diff, only : timeref
    use inputphysics_diff, only : equationmode
    implicit none
!
!      local variables.
...
!
    use constants
    use blockpointers_diff, only : nx, ny, nz, il, jl, kl, ie, je, ke, &
...
&   factfamilyk
    use flowvarrefstate_diff, only : pinfcorr, pinfcorrd
    ...
    use utils_diff, only : getcorrectfork, mydim, mydim_b
    implicit none
!
!      local parameters.
!
...
&   sfacei, sfacej, sfacek, factfamilyi, factfamilyj, factfamilyk
    use flowvarrefstate_diff, only : pinfcorr
    use inputdiscretization, only : vis2, vis4
    use inputphysics_diff, only : equations
    use iteration, only : rfil
    use cgnsgrid, only : massflowfamilydiss
...
&   radj, radjd, radk, radkd, gamma
    use flowvarrefstate_diff, only : gammainf, pinfcorr, pinfcorrd, &
&   rhoinf, rhoinfd
    use inputdiscretization, only : vis2, vis4
    use inputphysics_diff, only : equations
    use iteration, only : rfil
...
&   ib, jb, kb, w, p, pori, porj, pork, fw, radi, radj, radk, gamma
    use flowvarrefstate_diff, only : gammainf, pinfcorr, rhoinf
    use inputdiscretization, only : vis2, vis4
    use inputphysics_diff, only : equations
    use iteration, only : rfil
...
!
    use constants
    use blockpointers_diff, only : il, jl, kl, ie, je, ke, ib, jb, kb, w&
...
&   rotmatrixk, factfamilyi, factfamilyj, factfamilyk
    use flowvarrefstate_diff, only : kpresent, nw, nwf, rgas, rgasd, &
&   tref, trefd
...
&   factfamilyk
    use flowvarrefstate_diff, only : kpresent, nw, nwf, rgas, tref
    use inputdiscretization, only : limiter, lumpeddiss, precond, &
&   riemann, riemanncoarse, orderturb, kappacoef
    use inputphysics_diff, only : equations
    use iteration, only : rfil, currentlevel, groundlevel
...
! corner halo's.
      ...
      do k=kl,2,-1
        do i=il,2,-1
          ...
          call popreal8(rhoi)
          temp = w(i, jb, k, irho)
          wd(i, jb, k, irho) = wd(i, jb, k, irho) - one*rhoid/temp**2
          ...
          call popreal8(rhoi)
          temp = w(i, je, k, irho)
          wd(i, je, k, irho) = wd(i, je, k, irho) - one*rhoid/temp**2
          ...
          call popreal8(rhoi)
          temp = w(i, 1, k, irho)
          wd(i, 1, k, irho) = wd(i, 1, k, irho) - one*rhoid/temp**2
          ...
          call popreal8(rhoi)
          temp = w(i, 0, k, irho)
          wd(i, 0, k, irho) = wd(i, 0, k, irho) - one*rhoid/temp**2
        end do
      end do
      do k=kl,2,-1
        do j=jl,2,-1
          ...
          call popreal8(rhoi)
          temp = w(ib, j, k, irho)
          wd(ib, j, k, irho) = wd(ib, j, k, irho) - one*rhoid/temp**2
          ...
          call popreal8(rhoi)
          temp = w(ie, j, k, irho)
          wd(ie, j, k, irho) = wd(ie, j, k, irho) - one*rhoid/temp**2
          ...
          call popreal8(rhoi)
          temp = w(1, j, k, irho)
          wd(1, j, k, irho) = wd(1, j, k, irho) - one*rhoid/temp**2
          ...
          call popreal8(rhoi)
          temp = w(0, j, k, irho)
          wd(0, j, k, irho) = wd(0, j, k, irho) - one*rhoid/temp**2
        end do
      end do
      do k=kb,0,-1
        do j=jl,2,-1
      ...
      do j=jl,2,-1
        do i=il,2,-1
          do k=kl,1,-1
            ...
            rradd = (fis2*min3+sigma*fis4)*dis2d
            call popcontrol1b(branch)
            if (branch .eq. 0) then
              call popreal8(min3)
            else
      ...
      do k=kl,2,-1
        do i=il,2,-1
          do j=jl,1,-1
            ...
            wd(i, j, k, irho) = wd(i, j, k, irho) - ddwd
            rradd = (fis2*min2+sigma*fis4)*dis2d
            call popcontrol1b(branch)
            if (branch .eq. 0) then
              call popreal8(min2)
            else
      ...
      do k=kl,2,-1
        do j=jl,2,-1
          do i=il,1,-1
            ...
            wd(i, j, k, irho) = wd(i, j, k, irho) - ddwd
            rradd = (fis2*min1+sigma*fis4)*dis2d
            call popcontrol1b(branch)
            if (branch .eq. 0) then
              call popreal8(min1)
            else
