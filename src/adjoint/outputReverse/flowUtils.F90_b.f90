...
!
    use constants
    use inputphysics_diff, only : cpmodel
    use flowvarrefstate_diff, only : rgas, rgasd, gammainf
...
!
    use constants
    use inputphysics_diff, only : cpmodel
    use flowvarrefstate_diff, only : rgas, gammainf
    use utils_diff, only : terminate
    implicit none
!
!      subroutine arguments.
...
!
    use constants
    use cpcurvefits
    use flowvarrefstate_diff, only : tref, trefd, rgas, rgasd, gammainf
    use inputphysics_diff, only : cpmodel
    implicit none
    real(kind=realtype), intent(in) :: rho, p, u, v, w
    real(kind=realtype) :: rhod, pd, ud, vd, wd
...
!
    use constants
    use cpcurvefits
    use flowvarrefstate_diff, only : tref, rgas, gammainf
    use inputphysics_diff, only : cpmodel
    implicit none
    real(kind=realtype), intent(in) :: rho, p, u, v, w
...
!
    use constants
    use blockpointers_diff, only : ie, je, ke, w, wd, p, pd, aa, aad, &
&   gamma
...
!
    use constants
    use blockpointers_diff, only : ie, je, ke, w, p, aa, gamma
    use utils_diff, only : getcorrectfork
    implicit none
!
!      local variables.
...
!
    use constants
    use blockpointers_diff, only : w, wd, p, pd
    use flowvarrefstate_diff, only : rgas, rgasd, tref, trefd
    use inputphysics_diff, only : cpmodel, gammaconstant
    implicit none
!
!      subroutine arguments.
...
!
    use constants
    use blockpointers_diff, only : w, p
...
!
    ...
    use inputphysics_diff, only : cpmodel, gammaconstant
    use blockpointers_diff, only : w, p
    use flowvarrefstate_diff, only : kpresent, pinf, rgas, tref
    use cpcurvefits
    implicit none
!
!      subroutine arguments.
...
!                beta:out dragdirection:in-out liftdirection:in-out
  subroutine adjustinflowangle_b()
    use constants
    use inputphysics_diff, only : alpha, alphad, beta, betad, liftindex,&
...
&                 veldirfreestream, veldirfreestreamd, liftindex)
  ...
  subroutine adjustinflowangle()
    use constants
    use inputphysics_diff, only : alpha, beta, liftindex, &
