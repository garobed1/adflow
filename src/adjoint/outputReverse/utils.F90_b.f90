...
!   with respect to varying inputs: x y
  ...
  function getcorrectfork()
    use constants
    use flowvarrefstate_diff, only : kpresent
...
!
    use constants
    use blockpointers_diff, only : w, p, rlv, rev, gamma, x, d2wall, si,&
&   sj, sk, s, globalcell, bcdata, nx, il, ie, ib, ny, jl, je, jb, nz, &
&   kl, ke, kb, bcfaceid, addgridvelocities, sfacei, sfacej, sfacek, &
&   addgridvelocities
    use bcpointers_diff, only : ww0, ww1, ww2, ww3, pp0, pp1, pp2, pp3, &
...
&   iend, jstart, jend, isize, jsize
    use inputphysics_diff, only : cpmodel, equations
    implicit none
! subroutine arguments.
...
!                                                      *
    use constants
    use inputphysics_diff, only : lengthref, pointref, pointrefec, &
&   liftindex
    implicit none
...
! outputs
    ...
    ad = 0.0_8
    bd = 0.0_8
    ad(1) = ad(1) + b(2)*cd(3) - b(3)*cd(2)
    bd(2) = bd(2) + a(1)*cd(3) - a(3)*cd(1)
    ad(2) = ad(2) + b(3)*cd(1) - b(1)*cd(3)
    bd(1) = bd(1) + a(3)*cd(2) - a(2)*cd(3)
    cd(3) = 0.0_8
    ad(3) = ad(3) + b(1)*cd(2) - b(2)*cd(1)
    bd(3) = bd(3) + a(2)*cd(1) - a(1)*cd(2)
    cd(2) = 0.0_8
    cd(1) = 0.0_8
