...
!   gradient     of useful results: uref pref *w *dw actuatorregions.f
!                plocal
...
!                actuatorregions.f:incr plocal:in-out
...
! block pointers are already set.
    use constants
    use actuatorregiondata_diff
    use blockpointers_diff, only : volref, dw, dwd, w, wd
    use flowvarrefstate_diff, only : pref, prefd, uref, urefd
    use communication
...
! this actually gets the force
    ...
    prefd = prefd - sum(actuatorregions(iregion)%f*tempd)/pref
...
! block pointers are already set.
    use constants
    use actuatorregiondata_diff
    use blockpointers_diff, only : volref, dw, w
    use flowvarrefstate_diff, only : pref, uref
    use communication
    use iteration, only : ordersconverged
    implicit none
! input
