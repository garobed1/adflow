...
!                pref:incr rgas:incr rhoref:incr vars:incr localvalues:in-out
...
! integrate over the trianges for the inflow/outflow conditions.
    ...
    use sorting_diff, only : faminlist
...
&   computettot_b
    ...
    use utils_diff, only : mynorm2, cross_prod, cross_prod_b
    implicit none
...
&           mass_nxd + sf*ss(3)*sfacecoordrefd(3) + sf*ss(2)*&
&           sfacecoordrefd(2) + sf*ss(1)*sfacecoordrefd(1)
          massflowratelocald = massflowratelocald + overcellarea*ss(3)*&
&           mass_nzd + overcellarea*ss(2)*mass_nyd + overcellarea*ss(1)*&
&           mass_nxd + (uref*vzm-sfacecoordref(3))*mass_vzd + (uref*vym-&
&           sfacecoordref(2))*mass_vyd + (uref*vxm-sfacecoordref(1))*&
&           mass_vxd + mnm*mass_mnd + pm*mass_psd + am*uref*mass_ad + &
&           rhom*rhoref*mass_rhod + ttot*tref*mass_ttotd + ptot*pref*&
&           mass_ptotd + massflowrated
...
&           massflowratelocald - temp*tempd
          ...
          ssd(3) = ssd(3) + overcellarea*massflowratelocal*mass_nzd + sf&
&           *overcellarea*sfacecoordrefd(3) + 2*ss(3)*tempd0 + vzm*vnmd
          ssd(2) = ssd(2) + overcellarea*massflowratelocal*mass_nyd + sf&
&           *overcellarea*sfacecoordrefd(2) + 2*ss(2)*tempd0 + vym*vnmd
          ssd(1) = ssd(1) + overcellarea*massflowratelocal*mass_nxd + sf&
&           *overcellarea*sfacecoordrefd(1) + 2*ss(1)*tempd0 + vxm*vnmd
...
! integrate over the trianges for the inflow/outflow conditions.
    ...
    use blockpointers_diff, only : bctype
    use sorting_diff, only : faminlist
    use flowvarrefstate_diff, only : pref, pinf, rhoref, pref, timeref, &
...
&   localvalues, localvaluesd, famlist, sps)
    use constants
    use sorting_diff, only : faminlist
    use flowvarrefstate_diff, only : lref
    use inputphysics_diff, only : pointref, pointrefd
    use utils_diff, only : mynorm2, mynorm2_b, cross_prod, cross_prod_b
    implicit none
! input/output
    integer(kind=inttype), dimension(:, :), intent(in) :: conn
    integer(kind=inttype), dimension(:), intent(in) :: fams
...
! note: momentum forces have opposite sign to pressure forces
        ...
        mxd = mvd(1)
        xcd = fy*mzd - fz*myd
        fyd = xc*mzd + fvd(2) - zc*mxd
        ycd = fz*mxd - fx*mzd
        fxd = zc*myd - yc*mzd + fvd(1)
        zcd = fx*myd - fy*mxd
        fzd = yc*mxd - xc*myd + fvd(3)
        v1d = 0.0_8
        v2d = 0.0_8
        v3d = 0.0_8
        tempd = triarea*fzd
...
&   famlist, sps)
    ...
    use sorting_diff, only : faminlist
    use flowvarrefstate_diff, only : lref
    use inputphysics_diff, only : pointref
    use utils_diff, only : mynorm2, cross_prod
    implicit none
! input/output
    integer(kind=inttype), dimension(:, :), intent(in) :: conn
