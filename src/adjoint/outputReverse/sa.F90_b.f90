...
module sa_diff
  ...
  real(kind=realtype) :: cv13d, kar2invd, cw36d, cb3invd
...
!                *si *sj *sk
!   with respect to varying inputs: saconsts timeref *w *rlv *scratch
!                *vol *d2wall *si *sj *sk
!   rw status of diff variables: saconsts:out timeref:incr *w:incr
!                *rlv:incr *scratch:in-out *vol:incr *d2wall:out
!                *si:incr *sj:incr *sk:incr rsak:(loc) rsacv1:(loc)
!                rsacw1:(loc) rsacw2:(loc) rsacw3:(loc) rsact3:(loc)
!                rsact4:(loc) rsacrot:(loc) rsacb1:(loc) rsacb2:(loc)
!                rsacb3:(loc) cw36:(loc) cv13:(loc) kar2inv:(loc)
...
&         scratchd(i, j, k, idvt) + term2*tempd0
        ...
        term1d = tempd0
        term2d = temp*tempd0
        temp0 = (one-ft2)*fv2 + ft2
        dist2invd = (kar2inv*rsacb1*temp0-rsacw1*fwsa)*term2d
        tempd = dist2inv*term2d
        kar2invd = kar2invd + rsacb1*temp0*tempd
        rsacb1d = rsacb1d + kar2inv*temp0*tempd
        tempd0 = kar2inv*rsacb1*tempd
        rsacw1d = rsacw1d - fwsa*tempd
        fwsad = -(rsacw1*tempd)
        ft2d = (1.0-fv2)*tempd0
        fv2d = (one-ft2)*tempd0
        call popcontrol1b(branch)
        if (branch .ne. 0) then
          ft2d = ft2d - rsacb1*ss*term1d
          rsacb1d = rsacb1d + ss*(one-ft2)*term1d
          ssd = ssd + rsacb1*(one-ft2)*term1d
        end if
        termfwd = gg*fwsad
        temp0 = (one+cw36)/(gg6+cw36)
        if (temp0 .le. 0.0_8 .and. (sixth .eq. 0.0_8 .or. sixth .ne. int&
&           (sixth))) then
          tempd0 = 0.0_8
        else
          tempd0 = sixth*temp0**(sixth-1)*termfwd/(gg6+cw36)
        end if
        tempd = -(temp0*tempd0)
        gg6d = tempd
        ggd = termfw*fwsad + 6*gg**5*gg6d
        cw36d = cw36d + tempd0 + tempd
        rrd = (6*rr**5*rsacw2-rsacw2+1.0)*ggd
        rsacw2d = rsacw2d + (rr**6-rr)*ggd
        call popcontrol1b(branch)
        if (branch .eq. 0) rrd = 0.0_8
        temp0 = kar2inv*dist2inv/sst
        wd(i, j, k, itu1) = wd(i, j, k, itu1) + temp0*rrd
        tempd0 = w(i, j, k, itu1)*rrd/sst
        kar2invd = kar2invd + dist2inv*tempd0
        dist2invd = dist2invd + kar2inv*tempd0
        sstd = -(temp0*tempd0)
        call popcontrol1b(branch)
        if (branch .eq. 0) sstd = 0.0_8
        call popcontrol1b(branch)
        if (branch .ne. 0) then
          rsacrotd = rsacrotd + min1*sstd
          min1d = rsacrot*sstd
          call popcontrol1b(branch)
          if (branch .eq. 0) then
            y1d = min1d
...
&             + two*y1d/(2.0*sqrt(two*strainmag2))
        ...
        ssd = ssd + sstd
        wd(i, j, k, itu1) = wd(i, j, k, itu1) + fv2*kar2inv*dist2inv*&
&         sstd
        tempd0 = w(i, j, k, itu1)*sstd
        fv2d = fv2d + kar2inv*dist2inv*tempd0
        kar2invd = kar2invd + fv2*dist2inv*tempd0
        dist2invd = dist2invd + fv2*kar2inv*tempd0
        call popcontrol1b(branch)
        if (branch .eq. 0) then
          rsact3d = rsact3d + exp(-(rsact4*chi2))*ft2d
          tempd0 = exp(-(rsact4*chi2))*rsact3*ft2d
          rsact4d = rsact4d - chi2*tempd0
          chi2d = -(rsact4*tempd0)
        else
          chi2d = 0.0_8
        end if
        tempd0 = -(fv2d/(one+chi*fv1))
        tempd = -(chi*tempd0/(one+chi*fv1))
        chid = tempd0 + fv1*tempd
        fv1d = chi*tempd
        tempd = fv1d/(chi3+cv13)
        tempd0 = -(chi3*tempd/(chi3+cv13))
        chi3d = tempd + tempd0
        cv13d = cv13d + tempd0
...
&       rotrate(1)*omegaxd
      rsacw3d = 6*rsacw3**5*cw36d
      rsakd = -(2*one*kar2invd/rsak**3)
      rsacv1d = 3*rsacv1**2*cv13d
      rsacb3d = 0.0_8
      rsacb2d = 0.0_8
      call saassign_b()
    end if
...
!                *sk
!   with respect to varying inputs: saconsts *w *rlv *scratch *vol
!                *si *sj *sk
!   rw status of diff variables: saconsts:out *w:incr *rlv:incr
!                *scratch:in-out *vol:incr *si:incr *sj:incr *sk:incr
!                rsak:(loc) rsacv1:(loc) rsacw1:(loc) rsacw2:(loc)
!                rsacw3:(loc) rsact3:(loc) rsact4:(loc) rsacrot:(loc)
!                rsacb1:(loc) rsacb2:(loc) rsacb3:(loc) cb3inv:(loc)
!   plus diff mem management of: w:in rlv:in scratch:in vol:in
!                si:in sj:in sk:in
...
&       )
      ...
      tempd0 = ttp*cb3inv*cdpd
      tempd = (nup+(one+rsacb2)*nutp)*cdpd
      ttpd = cb3inv*tempd + cnud*capd
      cb3invd = cb3invd + ttp*tempd
      nupd = tempd0
      rsacb2d = rsacb2d + nutp*tempd0
      nutpd = (one+rsacb2)*tempd0
      tempd0 = ttm*cb3inv*cdmd
      tempd = (num+(one+rsacb2)*nutm)*cdmd
      ttmd = cb3inv*tempd + cnud*camd
      numd = tempd0
      rsacb2d = rsacb2d + nutm*tempd0
      nutmd = (one+rsacb2)*tempd0
      temp0 = w(i+1, j, k, irho)
      nud = half*nupd + half*numd
      cnudd = ttp*capd + ttm*camd
      tempd0 = -(w(i, j, k, itu1)*cnudd)
      cb3invd = cb3invd + ttm*tempd + rsacb2*tempd0
...
&       temp0
      temp = w(i, j, k-1, irho)
      cnudd = ttp*capd + ttm*camd
      tempd = -(w(i, j, k, itu1)*cnudd)
      rsacb2d = rsacb2d + nutm*tempd0 + cb3inv*tempd
