...
!
module surfaceintegrations_diff
  implicit none
! ----------------------------------------------------------------------
!                                                                      |
...
&   funcvaluesd)
    ...
    use flowvarrefstate_diff, only : pref, prefd, rhoref, rhorefd, tref,&
&   trefd, lref, gammainf, pinf, pinfd, uref, urefd, uinf, uinfd
    use inputphysics_diff, only : liftdirection, liftdirectiond, &
...
&   lengthref, alpha, alphad, beta, betad, liftindex
    ...
    use utils_diff, only : computetsderivatives
    use flowutils_diff, only : getdirvector
    implicit none
! input/output
...
&           globalvals(imassvy, sps)*mavgvyd/mflow**2 - globalvals(&
&           imassvx, sps)*mavgvxd/mflow**2 - globalvals(imassa, sps)*&
&           mavgad/mflow**2 - globalvals(imassmn, sps)*mavgmnd/mflow**2 &
&           - globalvals(imassps, sps)*mavgpsd/mflow**2 - globalvals(&
&           imassrho, sps)*mavgrhod/mflow**2 - globalvals(imassttot, sps&
...
&       , :) + forced
  ...
  subroutine getcostfunctions(globalvals, funcvalues)
    use constants
    use inputtimespectral, only : ntimeintervalsspectral
    use flowvarrefstate_diff, only : pref, rhoref, tref, lref, gammainf,&
&   pinf, uref, uinf
    use inputphysics_diff, only : liftdirection, dragdirection, &
&   surfaceref, machcoef, lengthref, alpha, beta, liftindex
    use inputtsstabderiv, only : tsstability
    use utils_diff, only : computetsderivatives
    use flowutils_diff, only : getdirvector
    implicit none
...
!
    use constants
    use communication
    use blockpointers_diff
    use flowvarrefstate_diff
    use inputcostfunctions
    use inputphysics_diff, only : machcoef, machcoefd, pointref, &
&   pointrefd, veldirfreestream, veldirfreestreamd, equations, &
&   momentaxis, cavitationnumber
    use bcpointers_diff
    implicit none
...
! sign into account (it should be a minus sign).
        ...
        m0yd = n(2)*tempd0
        m0zd = n(3)*tempd0
        fzd = bcdatad(mm)%fv(i, j, 3) + r(2)*m0xd - r(1)*m0yd + yc*mxd -&
&         xc*myd + blk*fvd(3)
        bcdatad(mm)%fv(i, j, 3) = 0.0_8
        fyd = bcdatad(mm)%fv(i, j, 2) + r(1)*m0zd + xc*mzd - r(3)*m0xd +&
&         blk*fvd(2) - zc*mxd
        bcdatad(mm)%fv(i, j, 2) = 0.0_8
        fxd = bcdatad(mm)%fv(i, j, 1) + r(3)*m0yd - r(2)*m0zd + zc*myd -&
&         yc*mzd + blk*fvd(1)
...
&       3))
      if (computecavitation) then
        ...
        temp0 = -(10*2*sensor1)
        temp = one + exp(temp0)
        sensor1d = 10*2*exp(temp0)*one*sensor1d/temp**2
        cpd = -sensor1d
      ...
      myd = blk*mpd(2)
      mzd = blk*mpd(3)
      sensord = zc*sepsensoravgd(3) + yc*sepsensoravgd(2) + xc*&
...
&         i, j, 3)**2))
      ...
      m0xd = n(1)*tempd
      m0yd = n(2)*tempd
      fzd = bcdatad(mm)%fp(i, j, 3) + r(2)*m0xd - r(1)*m0yd + yc*mxd - &
&       xc*myd + blk*fpd(3)
      bcdatad(mm)%fp(i, j, 3) = 0.0_8
      m0zd = n(3)*tempd
      fyd = bcdatad(mm)%fp(i, j, 2) + r(1)*m0zd + xc*mzd - r(3)*m0xd + &
&       blk*fpd(2) - zc*mxd
      bcdatad(mm)%fp(i, j, 2) = 0.0_8
      fxd = bcdatad(mm)%fp(i, j, 1) + r(3)*m0yd - r(2)*m0zd + zc*myd - &
&       yc*mzd + blk*fpd(1)
...
!
    ...
    use flowvarrefstate_diff
    use inputcostfunctions
    use inputphysics_diff, only : machcoef, pointref, veldirfreestream, &
...
!   gradient     of useful results: pointref timeref tref pref
!                rgas rhoref *xx *pp1 *pp2 *ssi *ww1 *ww2 localvalues
...
!                pref:incr rgas:incr rhoref:incr *xx:incr *pp1:incr
...
&   mm)
    use constants
    use blockpointers_diff, only : bctype, bcfaceid, bcdata, bcdatad, &
&   addgridvelocities
    use flowvarrefstate_diff, only : pref, prefd, pinf, pinfd, rhoref, &
&   rhorefd, timeref, timerefd, lref, tref, trefd, rgas, rgasd, uref, &
&   urefd, uinf, uinfd, rhoinf, rhoinfd
    use inputphysics_diff, only : pointref, pointrefd, flowtype
    use flowutils_diff, only : computeptot, computeptot_b, computettot, &
&   computettot_b
    use bcpointers_diff, only : ssi, ssid, sface, ww1, ww1d, ww2, ww2d, &
&   pp1, pp1d, pp2, pp2d, xx, xxd, gamma1, gamma2
    use utils_diff, only : mynorm2
    implicit none
! input/output variables
...
&       internalflowfact*inflowfact
      ...
      ycd = fz*mxd - fx*mzd
      fxd = zc*myd - yc*mzd + fmomd(1)
      zcd = fx*myd - fy*mxd
      fzd = yc*mxd - xc*myd + fmomd(3)
      ...
      tempd = fact*blk*internalflowfact*inflowfact*massflowratelocald/(&
&       timeref*cellarea)
      massflowratelocald = tempd
      tempd0 = -(massflowratelocal*tempd/(timeref*cellarea))
      timerefd = timerefd + cellarea*tempd0
      mzd = mpd(3)
      myd = mpd(2)
      mxd = mpd(1)
      fx = pm*ssi(i, j, 1)
      fy = pm*ssi(i, j, 2)
      fyd = xc*mzd + fpd(2) - zc*mxd
      fxd = zc*myd - yc*mzd + fpd(1)
      fz = pm*ssi(i, j, 3)
      xcd = xcd + fy*mzd - fz*myd
      ycd = ycd + fz*mxd - fx*mzd
      zcd = zcd + fx*myd - fy*mxd
      fzd = yc*mxd - xc*myd + fpd(3)
      ...
      call popreal8(pm)
      tempd = -(fact*blk*pmd)
      pmd = tempd + cellarea*blk*area_psd + massflowratelocal*mass_psd
      prefd = prefd - pinf*tempd
      tempd = fourth*zcd
      refpointd(3) = refpointd(3) - zcd
      xxd(i, j, 3) = xxd(i, j, 3) + tempd
      xxd(i+1, j, 3) = xxd(i+1, j, 3) + tempd
...
&       mass_nzd
      tempd = ssi(i, j, 3)*mass_nzd
      overcellaread = massflowratelocal*tempd
      massflowratelocald = massflowratelocald + overcellarea*tempd
      ssid(i, j, 2) = ssid(i, j, 2) + overcellarea*massflowratelocal*&
&       mass_nyd
      tempd = ssi(i, j, 2)*mass_nyd
      overcellaread = overcellaread + massflowratelocal*tempd
      massflowratelocald = massflowratelocald + overcellarea*tempd
      ssid(i, j, 1) = ssid(i, j, 1) + overcellarea*massflowratelocal*&
&       mass_nxd
      tempd = ssi(i, j, 1)*mass_nxd
      massflowratelocald = massflowratelocald + overcellarea*tempd + (&
&       uref*vzm-sfacecoordref(3))*mass_vzd + (uref*vym-sfacecoordref(2)&
&       )*mass_vyd + (uref*vxm-sfacecoordref(1))*mass_vxd + mnm*mass_mnd&
&       + pm*mass_psd + am*uref*mass_ad + rhom*rhoref*mass_rhod + ttot*&
&       tref*mass_ttotd + ptot*pref*mass_ptotd + massflowrated
      ...
      overcellaread = overcellaread + massflowratelocal*tempd + ssi(i, j&
&       , 3)*sf*sfacecoordrefd(3) + ssi(i, j, 2)*sf*sfacecoordrefd(2) + &
&       ssi(i, j, 1)*sf*sfacecoordrefd(1)
      ...
      cellaread = timeref*tempd0 + pm*blk*area_psd + ptot*pref*tempd + &
&       blk*aread - overcellaread/cellarea**2
...
&         i, j, 3)**2))
  ...
  subroutine flowintegrationface(isinflow, localvalues, mm)
    use constants
    use blockpointers_diff, only : bctype, bcfaceid, bcdata, &
&   addgridvelocities
    use flowvarrefstate_diff, only : pref, pinf, rhoref, timeref, lref, &
&   tref, rgas, uref, uinf, rhoinf
    use inputphysics_diff, only : pointref, flowtype
    use flowutils_diff, only : computeptot, computettot
    use bcpointers_diff, only : ssi, sface, ww1, ww2, pp1, pp2, xx, &
&   gamma1, gamma2
    use utils_diff, only : mynorm2
    implicit none
! input/output variables
...
end module surfaceintegrations_diff
