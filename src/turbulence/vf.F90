...
contains
  ...
  subroutine vfSolve(resOnly)
    ...
    use inputPhysics
    use paramTurb
    use turbMod, only : prod, dvt, sig1, sig2, sct, scl2
    use turbUtils, only : turbAdvection, unsteadyTurbTerm, tdia3
    ...
    do k=2,kl
       do j=2,jl
          do i=2,il
             ...
             dw(i,j,k,itu3) = -volRef(i,j,k)*dvt(i,j,k,1)*rblank
             dw(i,j,k,itu4) = -volRef(i,j,k)*dvt(i,j,k,2)*rblank
          enddo
       enddo
    enddo
  ...
  subroutine keSolve(resOnly)
    ...
    use inputPhysics
    use paramTurb
    use turbMod, only : prod, dvt, sct, scl2, sig1, sig2
    use turbUtils, only : turbAdvection, unsteadyTurbTerm, tdia3
    use turbCurveFits, only : curveTupYp
    implicit none
    !
    ...
    real(kind=realType) :: rhoi, ss, spk, ff1, ff2, ff3
    real(kind=realType) :: ss1, ss2, ss3
    real(kind=realType) :: voli, volmi, volpi
    real(kind=realType) :: xm, ym, zm, xp, yp, zp, xa, ya, za
    real(kind=realType) :: ttm, ttp, mulm, mulp, muem, muep
    real(kind=realType) :: c1m, c1p, c10, c2m, c2p, c20
    real(kind=realType) :: b1, b2, c1, c2, d1, d2
    real(kind=realType) :: qs, uu, um, up, utau
    real(kind=realType) :: tke, tep, tv2, tkea, tepa, tv2a
    real(kind=realType) :: tv2l, stei, sle2i
    real(kind=realType) :: rnu23, tu12, tu22, tu52, prod2, dtu23
    real(kind=realType) :: factor, rblank
    ...
    do k=2,kl
       do j=2,jl
          do i=2,il
             ...
             tepa = abs(tep)
             tv2a = abs(tv2)
             tv2l = max(tv2a,rvfLimitK*0.66666_realType)
             stei = tepa/sct(i,j,k)
             sle2i= tepa**2/scl2(i,j,k)
             ss   = prod(i,j,k)
             spk  = rev(i,j,k)*ss*rhoi
             ...
             ! alp = rvfN1A+rvfN1B &
             !     / (1.+(d2Wall(i,j,k)*.5*rvfCl)**2*sle2i)**4
             alp = rvfN1A+rvfN1B &
                  / (1.+d2Wall(i,j,k)**2*.25*rvfCl**2*tepa**2/scl2(i,j,k))**4
             if(rvfN == 6) alp = rvfN6A*(1.+rvfN6B*sqrt(tkea/tv2l))
             ...
             dvt(i,j,k,1) = ff1*tke + ff2*tep + ff3
             dvt(i,j,k,2) = ss1*tke + ss2*tep + ss3
    ...
    do k=2,kl
       do j=2,jl
          do i=2,il
             ...
             ! can be taken into account; the latter appears inside the
             ! d/dzeta derivative. The whole term is divided by rho to
             ! obtain the diffusion term for k and epsilon.
       ...
       bocos: do nn=1,nViscBocos
          ...
          do j=(BCData(nn)%jnBeg+1),BCData(nn)%jnEnd
             do i=(BCData(nn)%inBeg+1),BCData(nn)%inEnd
                ...
                ! Compute epsilon from balance

                call curveTupYp(tup(itu5:itu5), yp, itu5, itu5)
                tu52  = tup(itu5)*rlv2(i,j)
                dtu23 = (w3(i,j,itu1)-tu12) &
                     / (d2Wall3(i-1,j-1)-d2Wall2(i-1,j-1))
                rnu23 = half*( (tu52     +rlv2(i,j))/w2(i,j,irho) + &
                     (rev3(i,j)+rlv3(i,j))/w3(i,j,irho) )
                prod2 = tu52/w2(i,j,irho)*(utau**2*w2(i,j,irho) &
                     / (rlv2(i,j)+tu52))**2
                tu22  = prod2 + rnu23*dtu23/(two*d2Wall2(i-1,j-1))

                ! Set rhs to turbulence variables

                ! dvt2(i,j,1) = (tu12 - w2(i,j,itu1))/alfaTurb
                ! dvt2(i,j,2) = (tu22 - w2(i,j,itu2))/alfaTurb

                dvt2(i,j,1) = (tu12 - w2(i,j,itu1))
                dvt2(i,j,2) = (tu22 - w2(i,j,itu2))*0.01
    ...
    ! Take the local time step into account. Use characteristic
    ! time stepping, i.e. a matrix time step is used, where
    ! dt is the inverse of the central jacobian times the cfl
    ! number. To avoid a matrix inversion the following system
    ! is solved. (I/dt + cc + bb + dd)*dw = rhs. Due to the
    ! matrix time stepping I/dt = cc/cfl. As in the rest of the
    ! algorithm only the modified central jacobian is used,
    ! stored it now.
    ...
    factor = one
    if( wallFunctions ) factor = one/alfaTurb
